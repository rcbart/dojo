...
<li><b>Server → Session store:</b> save sid → {user, expiry} <i>(back channel)</i></li>
<li><b>Server → Browser:</b> Set-Cookie: sid=… HttpOnly Secure SameSite <i>(front channel)</i></li>
<li><b>Browser → Server:</b> GET /account, cookie attached automatically <i>(front channel)</i></li>
<li><b>Server → Session store:</b> look up sid <i>(back channel)</i></li>
<li><b>Server → Browser:</b> 200, personalized page <i>(front channel)</i></li>
</ol>
<!--/flow:ss1-session-->
...
tests:[{d:'joins name and value with =',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:name\\s*\\+\\s*"="\\s*\\+\\s*value))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:name\\s*\\+\\s*"="\\s*\\+\\s*value)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:name\\s*\\+\\s*"="\\s*\\+\\s*value)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:name\\s*\\+\\s*"="\\s*\\+\\s*value)[^{]*?return\\s+\\k<av>\\b)'},{d:'does not return null',re:'return\\s+null\\s*;',not:true}],
...
<li><b>evil.site → Victim’s browser:</b> page auto-submits a hidden form → bank.com <b>⚠ attack</b></li>
<li><b>Victim’s browser → bank.com:</b> POST /transfer, cookie attached AUTOMATICALLY <b>⚠ attack</b></li>
...
tests:[{d:'guards against a null token',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:cookieToken\\s*!=\\s*null))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:cookieToken\\s*!=\\s*null)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:cookieToken\\s*!=\\s*null)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:cookieToken\\s*!=\\s*null)[^{]*?return\\s+\\k<av>\\b)'},{d:'requires the tokens to match',re:'cookieToken\\.equals\\s*\\(\\s*formToken\\s*\\)'},{d:'combines both conditions',re:'&&'}],
...
tests:[{d:'an HttpOnly cookie is safe',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:"httponly-cookie"))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:"httponly-cookie")[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:"httponly-cookie")[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:"httponly-cookie")[^{]*?return\\s+\\k<av>\\b)'},{d:'does not bless localStorage',re:'"localstorage"',not:true}],
...
tests:[{d:'removes the session id from the active set',re:'active\\.remove\\s*\\(\\s*sid\\s*\\)'},{d:'confirms it is gone',re:'return\\s+!\\s*\\(?\\s*[^;{!]*?active\\s*\\.\\s*contains\\s*\\(\\s*sid\\s*\\)|(?<a1>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*!\\s*\\(?\\s*active\\s*\\.\\s*contains\\s*\\(\\s*sid\\s*\\)[^{]*?return\\s+\\k<a1>\\b|(?<a2>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{!]*active\\s*\\.\\s*contains\\s*\\(\\s*sid\\s*\\)[^{]*?return\\s+!\\s*\\k<a2>\\b|if\\s*\\(\\s*active\\s*\\.\\s*contains\\s*\\(\\s*sid\\s*\\)\\s*\\)\\s*\\{?\\s*return\\s+false|if\\s*\\(\\s*!\\s*active\\s*\\.\\s*contains\\s*\\(\\s*sid\\s*\\)\\s*\\)\\s*\\{?\\s*return\\s+true|if\\s*\\(\\s*!\\s*\\(\\s*!\\s*active\\s*\\.\\s*contains\\s*\\(\\s*sid\\s*\\)[^;{]*\\)*\\s*\\{?\\s*return\\s+false'}],
...
<li><b>OP (IdP) → App B:</b> POST logout_token <i>(back channel)</i></li>
<li><b>App A:</b> verify token; kill the server-side session</li>
...
tests:[{d:'the issuer must match',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:iss\\s*!=\\s*null|expectedIss\\s*\\.\\s*equals|iss\\s*\\.\\s*equals))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:iss\\s*!=\\s*null|expectedIss\\s*\\.\\s*equals|iss\\s*\\.\\s*equals)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:iss\\s*!=\\s*null|expectedIss\\s*\\.\\s*equals|iss\\s*\\.\\s*equals)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:iss\\s*!=\\s*null|expectedIss\\s*\\.\\s*equals|iss\\s*\\.\\s*equals)[^{]*?return\\s+\\k<av>\\b)'},{d:'the audience must match this client',re:'clientId|aud'},{d:'the logout event must be present',re:'hasLogoutEvent'},{d:'a nonce disqualifies the token',re:'!\\s*hasNonce|hasNonce\\s*==\\s*false'},{d:'replayed jtis are rejected',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:contains\\s*\\(\\s*jti\\s*\\)))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:contains\\s*\\(\\s*jti\\s*\\))[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:contains\\s*\\(\\s*jti\\s*\\))[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:contains\\s*\\(\\s*jti\\s*\\))[^{]*?return\\s+\\k<av>\\b)'},{d:'a null jti is rejected',re:'jti\\s*!=\\s*null|jti\\s*==\\s*null'},{d:'sid is preferred over sub',re:'sid\\s*!=\\s*null|null\\s*!=\\s*sid'},{d:'a guard returns its constant, not the negation of it',re:'return\\s+!\\s*\\(?\\s*(?:true|false)\\s*\\)?\\s*;',not:true}],
