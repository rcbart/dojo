...
tests:[{d:'a signing key is a total compromise',re:'"signing-key"'},{d:'an admin account is a total compromise',re:'"admin-account"'},{d:'refresh tokens are durable for one user',re:'"durable-user"'},{d:'client secrets affect the whole application',re:'"application"'},{d:'unknown inputs fall through',re:'"unknown"'},{d:'an access token cannot be revoked, but every other known radius can',re:'(?=[\\s\\S]*(?:if\\s*\\(\\s*("access-token"\\s*\\.\\s*equals\\s*\\(\\s*leaked|leaked\\s*\\.\\s*equals\\s*\\(\\s*"access-token")\\s*\\)\\s*\\)\\s*\\{?\\s*return\\s+false|case\\s*"access-token"\\s*(->|:)\\s*\\{?\\s*return\\s+false|return\\s+!\\s*\\(?\\s*[^;{!]*("access-token"\\s*\\.\\s*equals|equals\\s*\\(\\s*"access-token")))(?=[\\s\\S]*(?:return\\s+!\\s*\\(?\\s*[^;{!]*("unknown"\\s*\\.\\s*equals|equals\\s*\\(\\s*"unknown")|(?<k1>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*!\\s*\\(?\\s*("unknown"\\s*\\.\\s*equals|equals\\s*\\(\\s*"unknown")[^{]*?return\\s+\\k<k1>\\b|(?<k2>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{!]*("unknown"\\s*\\.\\s*equals|equals\\s*\\(\\s*"unknown")[^{]*?return\\s+!\\s*\\k<k2>\\b|if\\s*\\(\\s*("unknown"\\s*\\.\\s*equals|equals\\s*\\(\\s*"unknown")[^;{]*\\)\\s*\\{?\\s*return\\s+false|if\\s*\\(\\s*!\\s*\\(?\\s*[^;{!]*("unknown"\\s*\\.\\s*equals|equals\\s*\\(\\s*"unknown")[^;{]*\\)*\\s*\\{?\\s*return\\s+true|if\\s*\\(\\s*!\\s*\\(\\s*!\\s*\\(?\\s*[^;{!]*("unknown"\\s*\\.\\s*equals|equals\\s*\\(\\s*"unknown")[^;{]*\\)*\\s*\\{?\\s*return\\s+false))'},{d:'persistence hunting follows a total compromise',re:'"total"\\s*\\.\\s*equals|equals\\s*\\(\\s*"total"'}],
...
tests:[{d:'a null password is rejected',re:'(?:if\\s*\\(\\s*[^;{]*(?:presented\\s*==\\s*null|null\\s*==\\s*presented)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:return\\s+(?!\\s*!)[^;{]*(?:presented\\s*==\\s*null|null\\s*==\\s*presented))|(?:(?<h1>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:presented\\s*==\\s*null|null\\s*==\\s*presented)[^{]*?return\\s+\\k<h1>\\b)'},{d:'the modern algorithm is recognized',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:"argon2"))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:"argon2")[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:"argon2")[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:"argon2")[^{]*?return\\s+\\k<av>\\b)'},{d:'each hash goes to its own checker, un-negated',re:'(?=[\\s\\S]*(?:(?:return\\s+(?!\\s*!)[^;{]*(?:modernCheck))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:modernCheck)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:modernCheck)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<m1>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:modernCheck)[^{]*?return\\s+\\k<m1>\\b)))(?=[\\s\\S]*(?:(?:return\\s+(?!\\s*!)[^;{]*(?:legacyCheck))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:legacyCheck)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:legacyCheck)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<l1>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:legacyCheck)[^{]*?return\\s+\\k<l1>\\b)))'},{d:'rehash only after a successful login',re:'loginSucceeded'},{d:'already-modern hashes are not rehashed',re:'!\\s*"argon2"\\s*\\.\\s*equals|\\!"argon2"'},{d:'the new subject wins when present',re:'newSub\\s*!=\\s*null|null\\s*!=\\s*newSub'},{d:'otherwise fall back to the mapping table',re:'mapping\\s*\\.\\s*get\\s*\\('},{d:'a guard returns its constant, not the negation of it',re:'return\\s+!\\s*\\(?\\s*(?:true|false)\\s*\\)?\\s*;',not:true}],
...
tests:[{d:'cached keys are an acceptable source',re:'jwksReachable\\s*\\|\\|\\s*haveCachedKeys'},{d:'the signature must still verify',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:signatureValid))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:signatureValid)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:signatureValid)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:signatureValid)[^{]*?return\\s+\\k<av>\\b)'},{d:'combines availability AND validity',re:'&&'}],
...
tests:[{d:'passwords are never logged',re:'(?:case\\s*["\']password["\']|equals\\s*\\(\\s*["\']password["\']\\s*\\)|["\']password["\']\\s*\\.\\s*equals\\s*\\([^)]*\\)|[=!]==?\\s*["\']password["\']|includes\\s*\\(\\s*["\']password["\']\\s*\\)|contains\\s*\\(\\s*["\']password["\']\\s*\\))[^;}]*?return\\s+false\\b|(?:case\\s*["\']password["\']|equals\\s*\\(\\s*["\']password["\']\\s*\\)|["\']password["\']\\s*\\.\\s*equals\\s*\\([^)]*\\)|[=!]==?\\s*["\']password["\']|includes\\s*\\(\\s*["\']password["\']\\s*\\)|contains\\s*\\(\\s*["\']password["\']\\s*\\))[^;}]*?->\\s*(?:\\{\\s*)?(?:return\\s+)?false\\b'},{d:'access tokens are never logged',re:'"access_token"'},{d:'PKCE verifiers are never logged',re:'"code_verifier"'},{d:'biometric templates are never logged',re:'"biometric_template"'},{d:'other fields are loggable',re:'(?:default\\s*(?::|->)\\s*(?:\\{\\s*)?(?:return\\s+)?true\\b)|(?:else\\s*\\{?\\s*return\\s+true\\b)|(?:\\}\\s*return\\s+true\\s*;)'},{d:'redaction keeps only a short prefix',re:'substring\\s*\\(\\s*0\\s*,\\s*6\\s*\\)'},{d:'short or missing values are fully redacted',re:'"\\[redacted\\]"'},{d:'the failure reason must be present',re:'"reason"'},{d:'a correlation id must be present',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:"correlation_id"))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:"correlation_id")[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:"correlation_id")[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:"correlation_id")[^{]*?return\\s+\\k<av>\\b)'},{d:'a guard returns its constant, not the negation of it',re:'return\\s+!\\s*\\(?\\s*(?:true|false)\\s*\\)?\\s*;',not:true}],
...
tests:[{d:'wrong audience is a negative case',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:"wrong-audience"))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:"wrong-audience")[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:"wrong-audience")[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:"wrong-audience")[^{]*?return\\s+\\k<av>\\b)'},{d:'alg none is a negative case',re:'(?:case\\s*["\']alg-none["\']|equals\\s*\\(\\s*["\']alg-none["\']\\s*\\)|["\']alg-none["\']\\s*\\.\\s*equals\\s*\\([^)]*\\)|[=!]==?\\s*["\']alg-none["\']|includes\\s*\\(\\s*["\']alg-none["\']\\s*\\)|contains\\s*\\(\\s*["\']alg-none["\']\\s*\\))[^;}]*?return\\s+true\\b|(?:case\\s*["\']alg-none["\']|equals\\s*\\(\\s*["\']alg-none["\']\\s*\\)|["\']alg-none["\']\\s*\\.\\s*equals\\s*\\([^)]*\\)|[=!]==?\\s*["\']alg-none["\']|includes\\s*\\(\\s*["\']alg-none["\']\\s*\\)|contains\\s*\\(\\s*["\']alg-none["\']\\s*\\))[^;}]*?->\\s*(?:\\{\\s*)?(?:return\\s+)?true\\b'},{d:'the PKCE downgrade is a negative case',re:'"pkce-downgrade"'},{d:'cross-tenant access is a negative case',re:'"cross-tenant"'},{d:'validation cases belong at unit level',re:'(?:if\\s*\\(\\s*scenario\\s*=\\s*=\\s*null\\s*\\)[^;}]*?return\\s+["\']unit["\'])'},{d:'flow cases belong at integration level',re:'"integration"'},{d:'the happy path is the e2e case',re:'"e2e"'},{d:'the suite must cover the IDOR case',re:'contains\\s*\\(\\s*"cross-tenant"\\s*\\)'},{d:'a guard returns its constant, not the negation of it',re:'return\\s+!\\s*\\(?\\s*(?:true|false)\\s*\\)?\\s*;',not:true}],
...
tests:[{d:'redirect errors point at the registration',re:'(?:case\\s*["\']invalid-redirect-uri["\'][^;}]*?return\\s+["\']client-registration["\'])|(?:case\\s*["\']invalid-client["\'][^;}]*?return\\s+["\']client-registration["\'])'},{d:'login loops point at cookies',re:'"cookies"'},{d:'grant errors point at the token exchange',re:'"token-exchange"'},{d:'a 403 after login is authorization',re:'"authorization"'},{d:'unknown symptoms fall through',re:'"unknown"'},{d:'an IdP-side issue affects everyone everywhere',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:allUsers\\s*&&\\s*allApps|allApps\\s*&&\\s*allUsers))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:allUsers\\s*&&\\s*allApps|allApps\\s*&&\\s*allUsers)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:allUsers\\s*&&\\s*allApps|allApps\\s*&&\\s*allUsers)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:allUsers\\s*&&\\s*allApps|allApps\\s*&&\\s*allUsers)[^{]*?return\\s+\\k<av>\\b)'},{d:'expiry is suspected when nothing changed',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:nothingChanged))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:nothingChanged)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:nothingChanged)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:nothingChanged)[^{]*?return\\s+\\k<av>\\b)'}],
...
tests:[{d:'new artifacts mean compromise',re:'(?:if\\s*\\(\\s*newArtifactsCreated\\s*\\)[^;}]*?return\\s+["\']compromise["\'])'},{d:'uniform failure with no creation is an outage',re:'"outage"'},{d:'anything else needs investigation',re:'"investigate"'}],
...
tests:[{d:'an RP ID change is irreversible',re:'(?:case\\s*["\']rp-id-change["\']|equals\\s*\\(\\s*["\']rp-id-change["\']\\s*\\)|["\']rp-id-change["\']\\s*\\.\\s*equals\\s*\\([^)]*\\)|[=!]==?\\s*["\']rp-id-change["\']|includes\\s*\\(\\s*["\']rp-id-change["\']\\s*\\)|contains\\s*\\(\\s*["\']rp-id-change["\']\\s*\\))[^;}]*?return\\s+false\\b|(?:case\\s*["\']rp-id-change["\']|equals\\s*\\(\\s*["\']rp-id-change["\']\\s*\\)|["\']rp-id-change["\']\\s*\\.\\s*equals\\s*\\([^)]*\\)|[=!]==?\\s*["\']rp-id-change["\']|includes\\s*\\(\\s*["\']rp-id-change["\']\\s*\\)|contains\\s*\\(\\s*["\']rp-id-change["\']\\s*\\))[^;}]*?->\\s*(?:\\{\\s*)?(?:return\\s+)?false\\b'},{d:'a password rehash is irreversible',re:'"password-rehash"'},{d:'a subject format change is irreversible',re:'(?:case\\s*["\']sub-format-change["\']|equals\\s*\\(\\s*["\']sub-format-change["\']\\s*\\)|["\']sub-format-change["\']\\s*\\.\\s*equals\\s*\\([^)]*\\)|[=!]==?\\s*["\']sub-format-change["\']|includes\\s*\\(\\s*["\']sub-format-change["\']\\s*\\)|contains\\s*\\(\\s*["\']sub-format-change["\']\\s*\\))[^;}]*?return\\s+false\\b|(?:case\\s*["\']sub-format-change["\']|equals\\s*\\(\\s*["\']sub-format-change["\']\\s*\\)|["\']sub-format-change["\']\\s*\\.\\s*equals\\s*\\([^)]*\\)|[=!]==?\\s*["\']sub-format-change["\']|includes\\s*\\(\\s*["\']sub-format-change["\']\\s*\\)|contains\\s*\\(\\s*["\']sub-format-change["\']\\s*\\))[^;}]*?->\\s*(?:\\{\\s*)?(?:return\\s+)?false\\b'},{d:'other changes are reversible',re:'(?:default\\s*(?::|->)\\s*(?:\\{\\s*)?(?:return\\s+)?true\\b)|(?:else\\s*\\{?\\s*return\\s+true\\b)|(?:\\}\\s*return\\s+true\\s*;)'},{d:'the new value must be accepted alongside the old',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:newAcceptedAlongsideOld))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:newAcceptedAlongsideOld)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:newAcceptedAlongsideOld)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:newAcceptedAlongsideOld)[^{]*?return\\s+\\k<av>\\b)'},{d:'the old value must not be removed immediately',re:'!\\s*oldRemovedImmediately|oldRemovedImmediately\\s*==\\s*false'},{d:'authorization changes need an effective diff',re:'"authorization"'},{d:'a guard returns its constant, not the negation of it',re:'return\\s+!\\s*\\(?\\s*(?:true|false)\\s*\\)?\\s*;',not:true}],
...
tests:[{d:'every gate must pass',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:handlesLegacyTail\\s*&&))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:handlesLegacyTail\\s*&&)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:handlesLegacyTail\\s*&&)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:handlesLegacyTail\\s*&&)[^{]*?return\\s+\\k<av>\\b)'},{d:'residency is a gate',re:'meetsResidency'},{d:'audit evidence is a gate',re:'producesAuditEvidence'},{d:'phishing-resistant MFA is a gate',re:'phishingResistantMfa'},{d:'migration effort carries the heaviest weight',re:'3\\s*\\*\\s*migrationEffortInverse'},{d:'operational burden is weighted equally',re:'3\\s*\\*\\s*operationalEase'},{d:'a failed gate disqualifies rather than scores',re:'-1'}],
...
tests:[{d:'zero distance is not a detection',re:'(?:if\\s*\\(\\s*[^;{]*(?:distanceKm\\s*<=\\s*0|distanceKm\\s*===\\s*0)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:return\\s+(?!\\s*!)[^;{]*(?:distanceKm\\s*<=\\s*0|distanceKm\\s*===\\s*0))|(?:(?<h1>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:distanceKm\\s*<=\\s*0|distanceKm\\s*===\\s*0)[^{]*?return\\s+\\k<h1>\\b)'},{d:'the zero-time case is handled before dividing',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:minutesApart\\s*<=\\s*0|minutesApart\\s*===\\s*0))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:minutesApart\\s*<=\\s*0|minutesApart\\s*===\\s*0)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:minutesApart\\s*<=\\s*0|minutesApart\\s*===\\s*0)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:minutesApart\\s*<=\\s*0|minutesApart\\s*===\\s*0)[^{]*?return\\s+\\k<av>\\b)'},{d:'speed is distance over time',re:'(?:return\\s+(?!\\s*!)[^;{]*(?:distanceKm\\s*/))|(?:if\\s*\\(\\s*(?!\\s*!)[^;{]*(?:distanceKm\\s*/)[^;{]*\\)\\s*\\{?\\s*return\\s+true)|(?:if\\s*\\(\\s*!\\s*[^;{]*(?:distanceKm\\s*/)[^;{]*\\)\\s*\\{?\\s*return\\s+false)|(?:(?<av>[A-Za-z_$][\\w$]*)\\s*=(?!=)\\s*(?!\\s*!)[^;{]*(?:distanceKm\\s*/)[^{]*?return\\s+\\k<av>\\b)'},{d:'the threshold is compared, not hardcoded',re:'maxKmh'}],
