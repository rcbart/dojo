...
tests:[{d:'workforce is always public',re:'(?:case\\s*["\']workforce["\']|population\\s*===?\\s*["\']workforce["\']|["\']workforce["\']\\s*===?\\s*population|["\']workforce["\']\\s*\\.\\s*equals|equals\\s*\\(\\s*["\']workforce["\'])[^;}]*?(?:return\\s+|->\\s*)["\']public["\']'},{d:'a shared sector groups an organization',re:'sameOrganisation\\s*\\?\\s*["\']pairwise-same-sector["\']|!\\s*sameOrganisation\\s*\\?\\s*["\']pairwise["\']\\s*:\\s*["\']pairwise-same-sector["\']|if\\s*\\(\\s*sameOrganisation\\s*\\)\\s*\\{?\\s*return\\s+["\']pairwise-same-sector["\']|if\\s*\\(\\s*!\\s*sameOrganisation\\s*\\)\\s*\\{?\\s*return\\s+["\']pairwise["\'][^{]*?return\\s+["\']pairwise-same-sector["\']'},{d:'consumer defaults to pairwise',re:'"pairwise"'}],
