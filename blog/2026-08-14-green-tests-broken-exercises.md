...
description: "I had two CI gates and a passing build. I was proofreading the exam and proofreading the answer key, but nobody was working the questions. Here's the 68-line check that found four broken exercises and a bug in the grader itself."
date: 2026-08-14
...
A check that produces a report is a signal. A check that can refuse to let your work ship is a **gate**.
The difference only matters on the day someone is in a hurry, which is to say it matters constantly.
...
Load the answer key. Work the questions. Compare. It runs the 169 executed exercises (the rest of the
600 are graded structurally) through their 912 cases in about a second, and it exits non-zero if
anything disagrees, which makes it a gate rather than a report.
...
Yes, everyone knows `JSON.stringify` isn't deep equality. I knew it too. That line was written in the
platform's first week, it worked for every exercise that returned a number, a string or an array, and
nothing ever forced anyone to look at it again, until an exercise returned an object.
...
change. (`canon` still inherits `JSON.stringify`'s remaining quirks: `undefined` values are dropped and
`NaN` becomes `null`. But the gate and the in-browser grader share the same `eq`, so the two can no
...
None of this is specific to coding exercises. Anywhere one artifact asserts facts about another (a quiz
bank and its answer key, documentation with runnable examples, a config file and the schema that claims to
describe it), the same bug can live: both halves carefully reviewed, the agreement between them checked by
nobody.

Mine cost 68 lines and most of a Saturday. It found four broken exercises and one broken grader, and it
turned "the build is green" from a feeling into something I can actually point at. All it does is sit down
