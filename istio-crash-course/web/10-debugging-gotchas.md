...
- **DestinationRule subset missing.** A VirtualService routing to `subset: v2` with no matching
  ...
  catches this.
- **Route order.** In a VirtualService, the **first matching** `http` rule wins, so specific matches
...
- **Ambient docs**: <https://istio.io/latest/docs/ambient/>, with deployment and migration guides.
...
4. In ambient mode your weighted split doesn't apply. What's missing? *(A waypoint, since L7 rules need
