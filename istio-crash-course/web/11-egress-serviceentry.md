...
(`outboundTrafficPolicy: ALLOW_ANY`). Your pod can call `api.stripe.com` and it works, but the mesh
...
- Timeouts and `DestinationRule` circuit breaking on the entries that matter, because external services
