...
<li><b>The classpath</b> is where classes are found: an ordered list of directories and jars that <code>javac</code> searches at compile time and <code>java</code> searches at runtime. A <b>jar</b> is a zip of .class files.</li>
...
<p>The JDK's own classes (java.util, java.net.http) need no dependency. They ship with the runtime. Only <code>java.lang</code> needs no import.</p>

<h4>How the classpath is searched</h4>
<p>When the JVM needs <code>com.acme.Order</code> it asks the class loader, which walks the list and takes the
<b>first</b> match.</p>
...
is the classic "it works locally" bug: the local and CI classpath order differ.</p>
<p><b>An import is not a dependency.</b> The dependency is what put the
jar on the classpath. Removing an import never fixes a
...
<p>Every problem above comes from a flat, unverified list. <b>JPMS</b>, the Java Platform Module System that
arrived in Java 9, replaces it with a module graph that is checked at startup. A <code>module-info.java</code>
says which packages a module exports and which modules it needs, and the compiler enforces it. Missing dependencies fail immediately rather than at first use, and split packages are
rejected outright. Most applications still run on the classpath: the ecosystem migration is
...
<p>Every artifact in the world is addressed by <b>groupId : artifactId : version</b>. <code>groupId</code> is a reverse-domain namespace you control,
<code>artifactId</code> the module name, <code>version</code> the release. A <code>-SNAPSHOT</code>
suffix means "unreleased and mutable": Maven re-checks it periodically. A release version is
...
<p>A wrong scope produces a recognizable failure. <code>provided</code> where you
needed <code>compile</code> compiles fine and throws <code>NoClassDefFoundError</code> at runtime.
That error often means "a scope is wrong", not "a dependency is missing".</p>

<h4>Transitive dependencies and the nearest-wins rule</h4>
<p>Your dependencies have dependencies. Maven resolves the graph for you. When two paths
demand different versions of the same library it does <b>not</b> pick the newest. It picks the one
<i>nearest to your POM</i> in the tree, breaking ties by declaration order. <code>mvn dependency:tree</code> is the single most useful diagnostic
...
dependencies. Child modules inherit a consistent set and never declare versions at all.</p>

<h4>The lifecycle: order is not yours to choose</h4>
<p><code>mvn package</code> runs validate → compile → test → package, in
that order. Invoking a phase runs every phase before it. You do not describe <i>how</i> to build. You attach plugins to phases
and accept the sequence.</p>
<p><b><code>install</code> is not deployment.</b> It copies the artifact into
...
<p><b><code>clean</code> is not a ritual.</b> It deletes <code>target/</code>. Adding it to every
build "just in case" throws away incremental compilation and slows the loop. Use it when stale
output is suspected: after changing a plugin version, or when the build behaves in a way the
...
<p>Dependency <b>scopes</b> control the classpath (table below). Useful daily commands:</p>
...
<p>The fixed phase order buys one thing: <b>every Maven project builds
the same way</b>. A developer joining a project they have never seen runs <code>mvn verify</code> and it
works. The build is described by convention rather than by a script only its author understands.
...
<p>This mechanic explains almost every mysterious
...
your <b>POM</b>, and ties break in declaration order. The POM is your <code>pom.xml</code>, Maven's project file:
name, dependencies, plugins.</p>
<p>So a direct dependency always beats anything transitive. An old direct declaration will
...
exercise into a build failure.</p>

<h4>Commands to have in your fingers</h4>
...
module and only what it needs. <code>-X</code> when you need to see which plugin decided
what. <code>mvn install</code> writes to a shared local repository: convenient locally, and a
...
lets you program it. A Gradle
build can do anything, including becoming a bespoke program that only its author
understands.</p>
<p>The discipline: use plugins and conventions for everything you can, and treat
imperative logic in the build file as a smell. The Kotlin DSL (<code>build.gradle.kts</code>) helps
because it is statically typed. You get completion and compile errors instead of
...
<p>A build that never caches usually has a task with undeclared or non-deterministic inputs. A
timestamp in the output, an absolute path, a file read that Gradle was not told about.</p>

<h4><code>api</code> vs <code>implementation</code>: the real lesson here</h4>
...
<p>This compile avoidance is what makes large Gradle builds fast. With <code>implementation</code>,
editing a module recompiles its dependents only when its <i>public API</i> changed. Declaring
...
<h4>Two conveniences to adopt now</h4>
<p><b>Version catalogs</b> (<code>gradle/libs.versions.toml</code>) put every dependency coordinate and
version in one typed file. Upgrades are one edit and there is no drift between modules.
<b>Toolchains</b> (<code>java { toolchain { languageVersion = JavaLanguageVersion.of(21) } }</code>) make
the build download and use a specific JDK rather than whatever the machine happens to have. It is the same
...
<ul>
<li><b>Maven</b>: rigid lifecycle, XML, large ecosystem, very little to learn before your first build. Most enterprises (and most Spring docs) speak Maven.</li>
<li><b>Gradle</b>: programmable, faster thanks to incremental builds, cache and daemon, first-class for Android/Kotlin. Complexity can grow unchecked.</li>
...
<p>Multi-module: a parent POM (<code>&lt;packaging&gt;pom&lt;/packaging&gt;</code>, <code>&lt;modules&gt;</code>) or Gradle <code>settings.gradle</code> with <code>include 'api', 'core'</code>.</p>
<h4>How to make the choice</h4>
<p>For a typical Spring Boot service it rarely matters. Both work, and the
...
<p>Splitting a repository into modules buys enforced boundaries, faster incremental builds and independent
reuse. The first benefit is the real one: <code>core</code> cannot
depend on <code>web</code> if the build does not allow it. Module boundaries are the only architectural constraint
the compiler will enforce.</p>
<p>The trap is splitting by <i>layer</i> (<code>controllers</code>, <code>services</code>,
<code>repositories</code>). Those modules all change together on every feature and
...
problems from earlier, multiplied. Centralize: Maven's
<code>&lt;dependencyManagement&gt;</code> in the parent (or an imported BOM), Gradle's version catalog (<code>libs.versions.toml</code>) and
platform. Declare versions in one place. Modules declare only <i>what</i> they need, never
<i>which version</i>.</p>
<p>Inheriting <code>spring-boot-starter-parent</code> is this. It is not for the plugins but for a
...
machine, no network calls that can return different answers on different days. Keep it fast. A build
people avoid running stops catching things.</p>`,
...
<p>What a senior's "new project" checklist contains, and why:</p>
...
└── src/
    ...
    └── test/resources/</div>
<p><b>Under the hood</b>, <code>./mvnw verify</code> does this. The wrapper downloads the pinned Maven version. Maven reads the <b>POM</b>, the <code>pom.xml</code> that names the project, its dependencies and its plugins, and resolves the dependency graph: local cache <code>~/.m2</code> first, then Central. It builds the compile/test classpaths. Plugins bound to each lifecycle phase run in order: compiler, surefire tests, jar, failsafe.</p>
<p>The senior habits: a formatter and linter as build plugins (Spotless), so style debates end. Tests and CI wired before the first feature. One-command onboarding: <code>git clone && ./mvnw verify</code> must just work.</p>

<h4>Why the wrapper is not optional</h4>
<p>Committing <code>mvnw</code> or <code>gradlew</code> means a new machine builds with the <i>same</i> build tool version you used, downloaded
automatically. That covers a colleague's laptop, a CI runner, and your own machine in two years. Without it the README acquires a line saying "install Maven 3.9 first", and that line is
...
skipped.</li>
<li><b>Fail the build on what matters</b>: test failures, and ideally a coverage floor and a
vulnerability scan. A warning nobody is obliged to read is not a control.</li>
<li><b>.gitignore before the first commit.</b> It keeps build output and IDE noise out of review. Removing <code>target/</code> or a stray <code>.env</code>
...
<p>Maven's standard layout is the contract the plugins rely on: source in
<code>src/main/java</code>, tests in <code>src/test/java</code>, resources beside them. A
correctly laid out project needs almost no configuration. A project that fought the convention
