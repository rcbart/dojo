...
<p><b>What you build</b>: a production-style URL shortener. POST a long URL, get back <code>https://yourdomain/r/aB3x9</code>. Opening it 301-redirects and counts the click. Small enough to finish, real enough to include a database, migrations, Docker, and a deploy.</p>
<p><b>You will practice</b>: Spring Boot REST, JPA + Flyway, base62 encoding, HTTP redirects done right, Docker multi-stage builds, and your first real deploy. <b>JPA</b> is the Java Persistence API: the standard for mapping Java objects to database tables. You annotate a class and the provider (here Hibernate, under Spring Data) writes the SQL. Prereqs: Fundamentals, Spring Boot and Deploying streams.</p>
...
<p>Point <code>application.yaml</code> at it: url <code>jdbc:postgresql://localhost:5432/shorty</code>, user/pass from env with local defaults (the Deploying stream pattern).</p>
<p><b>Step 3, entity and repository.</b> <code>Link.java</code> is an @Entity on the links table: id @GeneratedValue, slug, target, clicks. <code>LinkRepository extends JpaRepository&lt;Link, Long&gt;</code> adds <code>Optional&lt;Link&gt; findBySlug(String slug)</code>. Multi-file starts here: one class per file, real packages (<code>dev.shorty.link</code>).</p>
<p><b>Step 4, the service (Exercise 1).</b> <code>ShortenService</code> turns a target URL into a stored Link with a random base62 slug, validating the URL first. Write it in the exercise tab, then drop the solution into the project.</p>
<p><b>Step 5, the API (Exercise 2).</b> <code>LinkController</code>: <code>POST /api/links</code> with body <code>{"url": "https://..."}</code> → 201 with <code>{"slug": "...", "target": "..."}</code>. <code>GET /r/{slug}</code> → <b>301 with a Location header</b> and a click increment. Unknown slug → 404. The status matters: 301 lets browsers cache the hop. Use 302 while debugging, 301 when it works.</p>
...
<p><b>Step 8: DEPLOY. The playbook every later project refers to.</b> Pick a lane:</p>
<ul>
<li><b>Option A: Hetzner VPS (most learning, ~€5/mo)</b>. Create a CX22 (Ubuntu 24.04, your SSH key) and run <code>curl -fsSL https://get.docker.com | sh</code>. Copy a production <code>compose.yaml</code>: your image from GHCR, postgres, caddy. Point a DNS A-record at the IP. A 2-line Caddyfile gives you automatic HTTPS: <code>shorty.yourdomain.dev { reverse_proxy app:8080 }</code>. Push-to-deploy comes free once you add the CI workflow from the CI/CD stream (build image → ssh <code>docker compose pull && up -d</code>).</li>
...
docs:[['Spring Initializr','https://start.spring.io'],['HTTP 301 vs 302 (MDN)','https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/301'],['Fly.io Dockerfile deploys','https://fly.io/docs/languages-and-frameworks/dockerfile/'],['Caddy reverse proxy','https://caddyserver.com/docs/quick-starts/reverse-proxy']],
exs:[
...
tests:[{d:'Repository constructor-injected into a final field',re:'private\\s+final\\s+LinkRepository\\s+repo[^{}]*?ShortenService\\s*\\(\\s*LinkRepository'},{d:'Rejects non-http(s) urls with IllegalArgumentException',re:'startsWith\\s*\\(\\s*"https?://"\\s*\\)(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?IllegalArgumentException|IllegalArgumentException(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?startsWith'},{d:'SecureRandom over the base62 alphabet constant',re:'(?=[\\s\\S]*SecureRandom)(?=[\\s\\S]*"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"\\s*[;.])'},{d:'Math.random / java.util.Random not used',re:'Math\\.random|new\\s+Random\\s*\\(',not:true},{d:'Collision loop consults findBySlug',re:'while\\s*\\([^{}]*?findBySlug[^{}]*?isPresent'},{d:'Saves and returns via the repository',re:'return\\s+repo\\.save\\s*\\(\\s*new\\s+Link'}],
...
hints:['The validation guard reads: if (url == null || !(url.startsWith("http://") || url.startsWith("https://"))) throw ...','randomSlug: loop 6 times appending ALPHABET.charAt(random.nextInt(ALPHABET.length())).','The collision loop is while (repo.findBySlug(slug).isPresent()) slug = randomSlug(); collisions are rare, but "rare" times a million links is "weekly".']},
...
tests:[{d:'POST /api/links returns 201',re:'@PostMapping\\s*\\(\\s*"/api/links"\\s*\\)[^{}]*?@ResponseStatus\\s*\\(\\s*HttpStatus\\.CREATED\\s*\\)|@ResponseStatus\\s*\\(\\s*HttpStatus\\.CREATED\\s*\\)[^{}]*?@PostMapping'},{d:'Body bound via @RequestBody record',re:'@RequestBody\\s+CreateLink'},{d:'GET /r/{slug} with @PathVariable',re:'@GetMapping\\s*\\(\\s*"/r/\\{slug\\}"\\s*\\)[^{}]*?@PathVariable'},{d:'301 MOVED_PERMANENTLY with Location header',re:'MOVED_PERMANENTLY(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?header\\s*\\(\\s*"Location"'},{d:'Click count incremented before redirecting',re:'getClicks\\s*\\(\\s*\\)\\s*\\+\\s*1'},{d:'Unknown slug → 404 via notFound()',re:'notFound\\s*\\(\\s*\\)\\s*\\.build'}],
...
hints:['Two annotations on create: @PostMapping for the route, @ResponseStatus(HttpStatus.CREATED) for the 201.','The redirect is ResponseEntity.status(HttpStatus.MOVED_PERMANENTLY).header("Location", target).build(); no body is needed.','Optional.map/orElse handles both branches without an if: map the present case to the 301, orElse the notFound().build().']},
...
<p><b>What you build</b>: a multi-user expense-tracking REST API with registration, JWT login, per-user data isolation, validation, real tests, and CI that blocks broken merges. A <b>JWT</b> is a JSON Web Token: a small signed document, three base64 pieces separated by dots, that carries claims such as who the user is and when the token expires. Anyone can read it; only the issuer can produce a valid signature. Here your server is the issuer, and the token is what a logged-in client sends with every later request. This is the "professional practices" project. Its skills transfer to nearly every backend job.</p>
...
<p><b>Step 1, skeleton.</b> start.spring.io as before with <code>web,data-jpa,postgresql,flyway,validation,security</code>, plus jjwt in the pom (io.jsonwebtoken jjwt-api/impl/jackson 0.12.x). Compose file with Postgres, same recipe as Shorty.</p>
...
<p><b>Step 3, auth.</b> Three pieces, each its own file. <code>AuthController</code>: register validates email and a 10+ char password, BCrypt-hashes, inserts, returns 201. Login verifies and mints a JWT (HS256, subject = user id, 7-day expiry, secret from <code>JWT_SECRET</code> env). <b>HS256</b> is the signing algorithm that uses one shared secret for both signing and verifying, so anyone who can verify can also forge. That is fine here, because the same server does both, and it is why the secret must never leak. <code>JwtFilter</code>: a OncePerRequestFilter that reads <code>Authorization: Bearer</code>, validates, and populates the SecurityContext. <code>SecurityConfig</code>: <b>Exercise 1</b>.</p>
<p><b>Step 4, the domain (Exercise 2).</b> <code>Expense</code> entity and repository with <code>findByUserId</code>/<code>findByIdAndUserId</code>. A record <b>DTO</b> with validation annotations: <code>@Positive</code> amountCents, <code>@NotBlank</code> category, <code>@PastOrPresent</code> spentOn. A DTO is a data transfer object: a plain class that only carries data across a boundary, such as the JSON an API sends or receives. No behavior, no database identity, so your internal model never leaks onto the wire. <code>ExpenseController</code>, where <b>every query is scoped to the authenticated user id</b>. This prevents IDOR (insecure direct object reference): <code>GET /api/expenses/17</code> must 404 when expense 17 is someone else's. With <code>findByIdAndUserId</code>, other users' rows are invisible.</p>
<p><b>Step 5, tests.</b> <code>ExpenseControllerTest</code> with MockMvc: anonymous request → 401; created-then-fetched roundtrip → 200 with the right JSON; fetching another user's expense id → 404. If you only write three tests in a project, write these three.</p>
<p><b>Step 6: CI (Exercise 3).</b> The workflow that runs the suite on every push and PR, with a Postgres <b>service container</b>, because your tests hit a real database, not H2 make-believe. <b>H2</b> is a small database written in Java that runs inside your test process. Fast tests, not production, and its quirks are not Postgres's quirks.</p>
<p><b>Step 7, deploy.</b> Playbook from Project 1 (Hetzner / Fly / Render), two deltas. Set <code>JWT_SECRET</code>, generated with <code>openssl rand -base64 32</code>, in a secret manager or platform secrets, never the compose file. Run migrations on boot (Flyway already does). <b>Done when</b> you can register on the public URL from curl, log in, post an expense from your phone, and a second account cannot see it.</p>
...
tests:[{d:'@Configuration + @EnableWebSecurity',re:'@Configuration[^{}]*?@EnableWebSecurity'},{d:'CSRF disabled for the stateless API',re:'csrf\\s*\\((?:[^()]|\\([^()]*\\))*?disable'},{d:'Session policy STATELESS',re:'SessionCreationPolicy\\.STATELESS'},{d:'Auth endpoints and health permitted to all',re:'requestMatchers\\s*\\(\\s*"/api/auth/\\*\\*"\\s*,\\s*"/actuator/health"\\s*\\)\\s*\\.permitAll'},{d:'Everything else authenticated',re:'anyRequest\\s*\\(\\s*\\)\\s*\\.authenticated'},{d:'JWT filter registered before the username/password filter',re:'addFilterBefore\\s*\\(\\s*jwtFilter\\s*,\\s*UsernamePasswordAuthenticationFilter\\.class'},{d:'BCrypt encoder bean exposed',re:'PasswordEncoder\\s+\\w+\\s*\\([^)]*\\)\\s*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?new\\s+BCryptPasswordEncoder\\s*\\(\\s*\\)'}],
...
tests:[{d:'List scoped to the caller: findByUserId',re:'findByUserId\\s*\\(\\s*userId\\s*\\)'},{d:'Create validates the body (@Valid) and returns 201',re:'@ResponseStatus\\s*\\(\\s*HttpStatus\\.CREATED\\s*\\)[^{}]*?@Valid\\s+@RequestBody|@Valid\\s+@RequestBody[^{}]*?CREATED'},{d:'Saved expense carries the CALLER’s id, not one from the body',re:'new\\s+Expense\\s*\\(\\s*userId'},{d:'Single fetch uses findByIdAndUserId (IDOR-proof)',re:'findByIdAndUserId\\s*\\(\\s*id\\s*,\\s*userId\\s*\\)'},{d:'Plain findById never used',re:'findById\\s*\\(',not:true},{d:'Missing/foreign row → 404',re:'notFound\\s*\\(\\s*\\)\\s*\\.build'}],
...
hints:['pull_request as a bare key (no filters) means every PR against any branch.','services: is a JOB-level key (a sibling of steps:); GitHub starts the container and wires networking before step 1.','The DB env vars go on the mvnw STEP (env: under that step), matching the names your application.yaml reads.']}
]},
{id:'prj3',title:'Project: DojoChat, realtime chat over WebSockets',body:`
<p><b>What you build</b>: a chat server with rooms. Open two browser tabs, type in one, see it in the other instantly. Then make it survive running on <i>two</i> server instances with Redis pub/sub. That is where it stops being a toy: state and concurrency become real.</p>
...
<p><b>Step 2, one instance, one room (Exercise 1).</b> <code>ChatHandler extends TextWebSocketHandler</code>: track live sessions in a <code>CopyOnWriteArraySet</code> and broadcast every incoming message to every open session. Register it in <code>WebSocketConfig implements WebSocketConfigurer</code> at path <code>/ws/chat</code>. <b>Each WebSocket session is driven by container threads concurrently</b>: a plain HashSet corrupts, an Iterator throws. The copy-on-write set is the right tool at chat scale: many reads and broadcasts, rare mutations.</p>
...
<p><b>Step 4, rooms.</b> Read the <code>room</code> query parameter from the session URI in <code>afterConnectionEstablished</code>. Keep <code>Map&lt;String, Set&lt;WebSocketSession&gt;&gt;</code> (ConcurrentHashMap + computeIfAbsent) and broadcast only within the room. Also handle <code>afterConnectionClosed</code>. Leaking closed sessions is THE classic WebSocket bug: every broadcast then throws on the dead session.</p>
...
<p><b>Step 7, deploy.</b> Playbook from Project 1, with the WebSocket deltas. <b>Hetzner/Caddy</b>: nothing extra. Caddy proxies WebSocket upgrades automatically (the reason we chose it). nginx would need explicit <code>Upgrade</code>/<code>Connection</code> headers. <b>Fly.io</b>: works out of the box, and <code>fly redis create</code> provides the bus. Scale to 2 machines (<code>fly scale count 2</code>) to see the pub/sub architecture earn its keep in production. <b>Render</b>: WebSockets supported on web services. Add their managed Redis. Use <code>wss://</code> (TLS) from the browser, because mixed content rules block <code>ws://</code> on an https page. <b>Done when</b> two phones on different networks chat through your domain.</p>`,
...
tests:[{d:'Publisher sends via convertAndSend to chat:<room>',re:'convertAndSend\\s*\\(\\s*"chat:"\\s*\\+\\s*room\\s*,\\s*message'},{d:'StringRedisTemplate constructor-injected, final',re:'private\\s+final\\s+StringRedisTemplate\\s+redis[^{}]*?ChatPublisher\\s*\\(\\s*StringRedisTemplate'},{d:'Listener container bean wired to the connection factory',re:'RedisMessageListenerContainer[^{}]*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?setConnectionFactory\\s*\\(\\s*factory'},{d:'Subscribes to the chat:* pattern',re:'PatternTopic\\s*\\(\\s*"chat:\\*"'},{d:'Incoming Redis messages delivered to local sessions',re:'deliverLocal\\s*\\(\\s*new\\s+String\\s*\\(\\s*message\\.getBody'}],
...
<p><b>What you build</b>: a weather dashboard for cities you pick. A Java backend consumes a public API (Open-Meteo, no key required), caches results, refreshes on a schedule, and serves a small live frontend. The consuming-external-APIs project: timeouts, caching, graceful degradation, daily bread in every integration job.</p>
<p><b>You will practice</b>: java.net.http.HttpClient, Jackson against JSON you do not control, Caffeine caching with TTLs, scheduled refresh, static file serving, defensive error handling. Prereqs: projects 1-2, APIs & REST and Caching lessons.</p>
<p><b>Step 1, skeleton.</b> start.spring.io with <code>web,cache</code> plus Caffeine (<code>com.github.ben-manes.caffeine:caffeine</code>) in the pom. No database: the cache IS the state, rebuildable from the source on restart (a fine property to recognize).</p>
...
<p><b>Step 4, cache + schedule (Exercise 2).</b> <code>WeatherService</code> with <code>@Cacheable("weather")</code> keyed by city. Caffeine configured <code>expireAfterWrite(10m), maximumSize(500)</code>. A <code>@Scheduled(fixedRate = 600_000)</code> refresher warms the configured cities so users rarely hit a cold path. Enable with <code>@EnableCaching</code> + <code>@EnableScheduling</code>. Cache even an API that changes hourly, because 1000 dashboard users must not become 1000 identical upstream calls. You are one <code>429 Too Many Requests</code> away from understanding rate limits personally.</p>
<p><b>Step 5, your API + frontend.</b> <code>GET /api/weather/{city}</code>: 404 for an unknown city, 503 when upstream fails and no cache exists. Plus <code>static/index.html</code>, a fetch loop every 60s rendering cards. Servers serve JSON. Browsers render it.</p>
<p><b>Step 6, resilience drill.</b> Kill your network and reload. The dashboard must keep showing cached readings (stale beats blank) and log upstream failures, not throw. For CI coverage of timeouts and 500s, test with WireMock: point the client's base URL at a mock that misbehaves on cue.</p>
<p><b>Step 7, deploy.</b> Playbook from Project 1, the easiest of the four: no database, no attachments, a single container on any of the three targets. One delta on Hetzner/Fly: set the JVM flag <code>-XX:MaxRAMPercentage=75</code> so the container's memory limit is respected. <b>Done when</b> your dashboard renders live cities on your public URL and survives an upstream outage gracefully. Extensions: city search (Open-Meteo geocoding API), a second source (air quality), an SSE endpoint pushing updates instead of the fetch loop.</p>`,
...
tests:[{d:'Shared client with a connect timeout',re:'HttpClient\\.newBuilder\\s*\\(\\s*\\)(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?connectTimeout\\s*\\(\\s*Duration\\.ofSeconds\\s*\\(\\s*3'},{d:'Per-request timeout set',re:'\\.timeout\\s*\\(\\s*Duration\\.ofSeconds\\s*\\(\\s*5'},{d:'Correct endpoint with both metrics requested',re:'api\\.open-meteo\\.com/v1/forecast\\?latitude=(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?current=temperature_2m,wind_speed_10m'},{d:'Non-200 becomes IllegalStateException with the code',re:'statusCode\\s*\\(\\s*\\)\\s*!=\\s*200(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?IllegalStateException(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?statusCode'},{d:'Tree-model parse of the current node',re:'readTree\\s*\\((?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?\\.path\\s*\\(\\s*"current"'},{d:'Returns your own Reading record, not their schema',re:'return\\s+new\\s+Reading\\s*\\('}],
...
tests:[{d:'Route with @PathVariable city',re:'@GetMapping\\s*\\(\\s*"/api/weather/\\{city\\}"\\s*\\)[^{}]*?@PathVariable'},{d:'Success wraps the reading in 200 OK',re:'ResponseEntity\\.ok\\s*\\(\\s*service\\.forCity'},{d:'Unknown city → 404',re:'catch\\s*\\(\\s*IllegalArgumentException[^{}]*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?notFound\\s*\\(\\s*\\)\\s*\\.build'},{d:'Upstream failure → 503, not 500',re:'catch\\s*\\(\\s*Exception[^{}]*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?SERVICE_UNAVAILABLE'},{d:'Catch order: specific before general',re:'catch\\s*\\(\\s*IllegalArgumentException[\\s\\S]*?catch\\s*\\(\\s*Exception'}],
