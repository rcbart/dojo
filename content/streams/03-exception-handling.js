...
<p>🌱 <b>Starting from zero:</b> things go wrong while programs run. A file is missing, text that should be a number reads "abc", the network dies mid-request. An <b>exception</b> is Java's fire alarm. When an operation can't do its job, it stops normal execution and throws an alarm object up to whoever is prepared to handle it. Prepared code says "try this, and if that alarm goes off, do this instead", which is how <code>try/catch</code> reads.</p>
<p>All throwables descend from <code>Throwable</code>: <code>Error</code> (JVM-level, don't catch), and <code>Exception</code>. <b>Checked</b> exceptions (e.g. <code>IOException</code>) must be caught or declared with <code>throws</code>. <b>Unchecked</b> ones (<code>RuntimeException</code> and subclasses like <code>NullPointerException</code>, <code>IllegalArgumentException</code>) need no declaration. They usually signal bugs or bad input.</p>
...
<p>The distinction is about <b>who can do something about it</b>. A checked exception is a documented, expected outcome the caller may recover from: the file might not be there, the remote service might be down. An unchecked exception says the program is wrong: a null where one wasn't allowed, an argument outside its contract. The compiler enforces the first and ignores the second. The choice belongs to the API designer.</p>
<p>So catch what you can act on. A <code>catch (Exception e)</code> at the top of a method swallows the NullPointerException that means you have a bug, alongside the IOException you meant to retry. The bug then surfaces days later as missing data rather than a stack trace.</p>

<h4>finally, and the two ways to lose an exception</h4>
<p><code>finally</code> runs whether the block completes, throws, or returns. That makes it the right place for cleanup and the wrong place for control flow. Two traps:</p>
...
<p>A <code>return</code> inside <code>finally</code> replaces whatever was in flight, including an exception on its way up. Rethrowing without passing the cause (<code>new IllegalStateException("failed", e)</code>) throws away the stack trace that says what went wrong. Both compile and both look defensive.</p>

<h4>Reading a stack trace</h4>
<p>The top line is what was thrown and where. Each line below is the caller beneath it. <code>Caused by:</code> sections read the same way, and the <b>deepest</b> cause is usually the real one. Frames marked <code>... 24 more</code> are shared with the trace above. Scroll to the last <code>Caused by</code> first, then find the topmost frame in <i>your</i> package. That's the line to open.</p>`,
...
<p>Extend <code>Exception</code> for recoverable, caller-must-decide situations. Extend <code>RuntimeException</code> for programming errors. Always pass a useful message, and the cause when wrapping.</p>

<h4>The real question: can the caller do something about it?</h4>
<p>That test decides checked versus unchecked. If a
caller could <i>recover</i> (retry, fall back, ask the user again), a checked exception
forces them to confront it. If the only sane response is to fix the code, make it unchecked.</p>
...
quantity" tells them nothing. <code>"quantity must be &gt; 0, was -3"</code> ends the
...
A message quoting a password or a token has just published it.</p>
...
<p>Dropping the cause is the most damaging mistake in this area. The original stack trace, the
line number and the driver's own message all disappear.</p>

<h4>What <code>throws</code> is really declaring</h4>
<p>It's part of your <b>API contract</b>, not an implementation detail. Declaring <code>throws
SQLException</code> on a repository method leaks the fact that you use a database into every caller.
The day you switch to an HTTP service, every signature changes. Wrap it in an exception that
belongs to your layer.</p>
<p>Overriding methods may narrow but never
widen the checked exceptions they declare, because callers were promised the parent's list. And
<code>throws Exception</code> is nearly always a mistake. It declares everything and so tells
the caller nothing.</p>

<h4>The anti-patterns, briefly</h4>
<ul>
<li><b>Swallowing</b>: <code>catch (Exception e) {}</code>. The failure still happened. You've only
hidden it. If it truly is ignorable, say so in a comment and log at debug.</li>
<li><b>Catching to log and rethrow</b>: the same error logged three times at three layers.
...
<p>🌱 <b>Starting from zero:</b> some things you open must be closed (files, network connections, database handles), like library books that must come back or nobody else can borrow them. The bug is that an exception mid-work skips your closing line. Java's fix is a try block that <b>closes automatically</b>, no matter how the block exits.</p>
<p>Anything implementing <code>AutoCloseable</code> can be opened in the try header. It's closed automatically, in reverse order, even when an exception flies. This replaces fragile <code>finally { x.close(); }</code> blocks.</p>
...
<p>If both the body and <code>close()</code> throw, the close-exception is attached as a <i>suppressed</i> exception instead of swallowing the original.</p>

<h4>What the compiler generates for you</h4>
<p>The try header expands to a nested structure that closes every resource in <b>reverse order of declaration</b> and guards each close against the others failing. The hard part by hand: it keeps the original exception primary and attaches any close failure as a <b>suppressed</b> exception, retrievable with <code>getSuppressed()</code>.</p>
<p>The naive manual version loses the original. If the body throws and then <code>close()</code> throws inside <code>finally</code>, the close exception replaces the real cause and you debug the wrong thing.</p>

<h4>The rules that catch people</h4>
<ul>
<li><b>Resources close in reverse order.</b> That's what you want when a writer wraps a stream: the wrapper flushes before the thing it wraps is closed.</li>
<li><b>Only what you open</b> belongs in the header. Putting a resource you were handed there closes something the caller still owns.</li>
<li><b>Java 9+ accepts an effectively-final variable</b> directly: <code>try (client; in; out)</code>. The socket lessons use this to adopt an already-created socket.</li>
...
<p>An empty <code>catch</code> removes the <b>evidence</b>, not the problem. The program
continues with a broken assumption, and the failure surfaces later, somewhere unrelated, as corrupt data or
a null nobody can explain. A crash tells you what went wrong and where.</p>
...
<p>In that last line the exception is passed as the <b>last argument</b>, not concatenated
into the message, so the logging framework records the full stack. Every catch block ends in a
decision: recover, translate and rethrow, or let it go.</p>
...
traces of the same event, and the log becomes unreadable during the incident. Handle
it where you can do something. Elsewhere, add context and pass it on.</p>

<h4>Wrapping, and why the cause matters</h4>
<p>Letting <code>SQLException</code> escape a repository means every caller depends on the storage
technology. Swap to a document store and the signatures change everywhere.</p>
<p>Translation without the cause is worse than no translation. You've replaced a precise error with a
vague one and lost the line that said what happened. Always pass the original as the
<code>cause</code>. <code>getCause()</code> is what you unwrap when a framework has wrapped
...
entry points and at construction, not deep inside.</p>

<h4>Two details that catch people</h4>
<p><b>try-with-resources over <code>finally</code>.</b> Closing in a <code>finally</code> has a real bug.
If the body throws and <code>close()</code> also throws, the close exception replaces the original.
try-with-resources closes in reverse order and attaches the secondary as a
...
which destroys a cancellation signal the rest of the system relies on. Restore it with
...
<p>Always offer a constructor that accepts a <b>cause</b> and pass it to <code>super(message, cause)</code>, so the original stack trace isn't lost (exception chaining).</p>

<h4>Designing the hierarchy, not just the class</h4>
<p>One custom exception is rarely enough and thirty is unusable. The pattern that scales is a small tree: one base exception per module or bounded context, with specific subtypes beneath it. Callers who care about a particular failure catch the subtype. Callers who only need "the payment layer failed" catch the base. Frameworks map the base to a status code in one place. Without the base class, every handler becomes a list of catch blocks that must change whenever anyone adds a failure mode.</p>

<h4>Carrying facts, not prose</h4>
<p>A message is for a human reading a log. A <b>field</b> is for code making a decision. <code>InsufficientFundsException</code> carrying <code>balance</code> and <code>required</code> lets a handler tell the user how much is missing, decide whether to offer a top-up, and emit a metric. Not possible if the numbers live only inside a formatted string. Add fields and getters for the context a handler will want (an id, an amount, a limit), so <code>catch</code> blocks act on facts instead of parsing strings.</p>
<p>Keep secrets and personal data <i>out</i> of exception messages. Exception text ends up in logs, in error trackers and sometimes in HTTP responses. A message including a token or a full card number has just published it three times over.</p>

<h4>The boundary rule</h4>
<p>Letting <code>SQLException</code> escape a repository, or a JSON parsing error escape a client, leaks the implementation into every caller. Replacing the database then changes the exception every layer catches. Translate at the boundary: catch the technology-specific exception, wrap it in a domain one <b>with the original as the cause</b>, and let it travel. Spring's <code>DataAccessException</code> hierarchy is this pattern applied to persistence.</p>
<p>Finally, cost. Filling in a stack trace is the expensive part of throwing. Using an exception as ordinary control flow at high frequency is rarely a good idea, but sometimes unavoidable in parsing. There, a constructor calling <code>super(msg, cause, false, false)</code> disables suppression and the stack trace. That makes it about as cheap as a return value.</p>`,
