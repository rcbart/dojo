...
`,
tests:[{d:'Q1: INSERT names its columns and its values',re:'(?=[\\s\\S]*(\'Effective Java\'[^)]{0,40}4500|4500[^)]{0,40}\'Effective Java\'))1\\)[\\s\\S]*?insert\\s+into\\s+books\\s*\\(\\s*author_id\\s*,\\s*title\\s*,\\s*price_cents\\s*\\)\\s*values',flags:'is'},{d:'Q2: one statement, two value tuples, each priced',re:'(?=[\\s\\S]*(\'Clean Code\'[^)]{0,40}3900|3900[^)]{0,40}\'Clean Code\'))(?=[\\s\\S]*(\'Refactoring\'[^)]{0,40}4700|4700[^)]{0,40}\'Refactoring\'))2\\)[\\s\\S]*?values[\\s\\S]*?\\(\\s*2\\s*,[\\s\\S]*?\\)\\s*,\\s*\\(\\s*2\\s*,',flags:'is'},{d:'Q3: RETURNING id fetches the generated key',re:'(?=[\\s\\S]*(\'DDIA\'[^)]{0,40}5200|5200[^)]{0,40}\'DDIA\'))3\\)[\\s\\S]*?insert[\\s\\S]*?returning\\s+id',flags:'is'},{d:'Q4: targeted update by primary key',re:'4\\)[\\s\\S]*?update\\s+books\\s+set\\s+price_cents\\s*=\\s*3990\\s+where\\s+id\\s*=\\s*7',flags:'is'},{d:'Q5: expression update reads the old value, scoped by WHERE',re:'5\\)[\\s\\S]*?set\\s+price_cents\\s*=\\s*price_cents\\s*\\*\\s*0?\\.9\\s+where\\s+author_id\\s*=\\s*1',flags:'is'},{d:'Q6: DELETE is scoped',re:'6\\)[\\s\\S]*?delete\\s+from\\s+books\\s+where\\s+price_cents\\s*=\\s*0',flags:'is'},{d:'Every UPDATE and DELETE carries a WHERE',re:'(update|delete)(?![\\s\\S]*?where)[^;]*;',not:true,flags:'i'}],
...
DROP TABLE tags;
`,
tests:[{d:'Q1: CREATE TABLE tags (DDL)',re:'1\\)[\\s\\S]*?create\\s+table\\s+tags',flags:'is'},{d:'Q2: ALTER TABLE ADD COLUMN slug',re:'2\\)[\\s\\S]*?alter\\s+table\\s+tags\\s+add\\s+column\\s+slug',flags:'is'},{d:'Q3: INSERT INTO tags with both values',re:'3\\)[\\s\\S]*?insert\\s+into\\s+tags[\\s\\S]{0,160}?\'java\'\\s*,\\s*\'java\'',flags:'is'},{d:'Q4: GRANT SELECT ... TO reader (DCL)',re:'4\\)[\\s\\S]*?grant\\s+select\\s+on\\s+tags\\s+to\\s+reader',flags:'is'},{d:'Q5: BEGIN ... UPDATE ... COMMIT (TCL)',re:'5\\)[\\s\\S]*?begin\\s*;[\\s\\S]*?update\\s+tags\\s+set\\s+slug\\s*=\\s*\'jvm\'\\s+where\\s+name\\s*=\\s*\'java\'[\\s\\S]*?commit\\s*;',flags:'is'},{d:'Q6: TRUNCATE tags (DDL)',re:'6\\)[\\s\\S]*?truncate\\s+tags',flags:'is'},{d:'Q7: DROP TABLE tags (DDL)',re:'7\\)[\\s\\S]*?drop\\s+table\\s+tags',flags:'is'}],
