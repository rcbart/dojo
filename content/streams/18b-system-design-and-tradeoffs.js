...
hints:['Context answers "why did this need deciding?" — one paragraph of problem, zero solution.','The Decision section is 2-4 sentences; if it sprawls, it is re-arguing instead of recording.','A Consequences section with only + lines is marketing, not engineering — the honest minus is what makes the doc trustworthy.']},{title:'ADR: Kafka vs a managed queue',lang:'text',
...
`,
tests:[{d:'Numbered title names the managed-queue decision',re:'#\\s*ADR-0002:.*(managed|cloud|sqs)',flags:'i'},{d:'Status accepted',re:'Status:\\s*accepted',flags:'i'},{d:'Context names the small team / no ops capacity',re:'##\\s*Context[\\s\\S]*?(small|three|no\\s+op|capacity)',flags:'i'},{d:'Decision defers Kafka with an explicit revisit trigger',re:'##\\s*Decision[\\s\\S]*?(deferred|revisit)[\\s\\S]*?(replay|consumers|volume|msgs)',flags:'i'},{d:'At least one honest plus',re:'##\\s*Consequences[\\s\\S]*?^\\+',flags:'im'},{d:'At least two honest minuses',re:'^-[\\s\\S]*?^-',flags:'im'}],
behavior:`1. The Context sells nothing — it states team size, volume and absent requirements, which make the decision nearly self-evident (good ADRs often do). 2. "Deferred, not rejected" with a NAMED trigger is the senior signature: the Kafka advocates know exactly what future evidence reopens the question, so the debate ends instead of festering. 3. Two minuses, honestly priced: lock-in and no-replay are real, and writing them down now is what makes the 2027 migration discussion start from facts instead of blame. 4. This ADR takes ten minutes and saves the third re-litigation of this argument in standup.`,
