...
<p>Everything before this stream taught you to build correctly. Seniority starts with a different question: <b>of the five correct designs, which one should exist?</b> The answer is always a tradeoff. The senior habit is naming what you are paying and what you are buying, out loud, before committing.</p>
...
<p><b>Back-of-envelope estimation</b> grounds these choices in numbers instead of vibes. The multiplication is deliberately crude. You want the order of magnitude, not the third digit:</p>
...
<p>Useful constants to carry. A day is ~100k seconds (86,400; round up for margin). Reads dominate writes 10:1 to 100:1 in most consumer systems. A request touching only RAM/cache is ~100× cheaper than one touching disk, and ~10× again versus one crossing a datacenter. Precision is not the goal. "1k req/s" and "1M req/s" are <i>different projects</i>, and five minutes of arithmetic tells you which one you are in before you design the wrong one.</p>`,
...
<ul>
<li><b>1. Vertical scaling</b>: a bigger box. Unfashionable and correct: zero code changes, and modern boxes are big. Its limits (price curve, single point of failure) are real but usually further away than people claim.</li>
...
<li><b>6. Sharding</b>: split the data itself across databases by some key. <b>Last resort</b>: cross-shard queries, rebalancing, and hot keys (the celebrity problem) make everything harder forever. Reach for it when a single writer cannot cope, not before.</li>
</ul>
...
<p>The meta-rule: <b>scale the bottleneck, not the architecture</b>. Measure, find the constraint, apply the cheapest tool that moves it, re-measure. Systems that jumped to microservices-plus-sharding on day one carry the tax forever while their traffic would have fit in a cache.</p>`,
...
<p>Most system-design decisions are secretly data decisions. The senior question is <b>"what are the queries, and what shape serves them?"</b> "SQL or NoSQL?" in the abstract is the wrong question.</p>
<ul>
<li><b>Start relational.</b> Postgres handles the first several orders of magnitude of almost anything, with transactions, joins, constraints and 50 years of tooling. Choosing a specialty store first and rediscovering joins by hand in application code is the most common self-inflicted wound in system design. Document stores earn their place when data is document-shaped and join-free. Key-value stores when access is only ever by key at huge scale. Search indexes for text search. Each is an <i>addition</i> justified by a query pattern, not a replacement chosen by fashion.</li>
<li><b>Normalization is for writes. Denormalization is for reads.</b> Third-normal-form (each fact stored once) makes updates cheap and anomalies impossible: the right default. But a feed that joins six tables per page view at 1,000 req/s may earn a denormalized copy: store the feed pre-built, and accept that a username change now means updating N rows. You are trading <b>write complexity for read speed</b>. Say so, and write down who updates the copy.</li>
<li><b>Indexes are the same trade in miniature.</b> Every index makes some query O(log n) and every write slightly slower. Index the columns your WHERE/ORDER BY use (your EXPLAIN lesson). <b>EXPLAIN</b> asks the database how it plans to run a query, and <code>EXPLAIN ANALYZE</code> runs it and reports what each step took; it is the first thing to read when a query is slow and the way to see whether an index is used at all. An unused index is pure write tax.</li>
<li><b>Cache vs denormalize vs precompute</b>: three answers to "reads are slow", in increasing commitment. A cache is disposable (it has a <b>TTL</b>, a time to live: how long a cached record is good for before it must be refreshed or thrown away; and it is rebuildable and can be dropped). A denormalized column is a schema promise, maintained forever. A precomputed table (materialized view, nightly rollup) sits between. Prefer the most disposable thing that meets the latency target.</li>
<li><b>Estimate the working set.</b> 36 TB/year sounds scary until you ask what is <i>hot</i>. If 95% of reads touch the last 30 days (~3 TB), that fits in RAM across a few replicas. Archive the rest to object storage. Hot/cold separation often deletes an order of magnitude from the hardware bill.</li>
</ul>
<p>The habit that binds these: <b>list the top 5 queries before choosing anything</b>, with their frequency, latency budget, and consistency needs. The design falls out of that table.</p>`,
...
<p>The method, start to finish, on a concrete problem: <b>design an image-sharing feed</b> (users post images, follow others, see a feed). This five-step loop works on any system, and it is the structure interviewers listen for.</p>
<p><b>1. Requirements & numbers first.</b> Functional: post image, follow, view feed. Non-functional: feed loads &lt; 200ms, 10M users, ~1M posts/day, reads dominate ~100:1. Deductions: 1M/day ≈ 12 writes/s (tiny!) but ~1,200 feed reads/s. <i>This system is a read problem</i>. Half the design just fell out of arithmetic.</p>
...
<p><b>4. The core decision.</b> Every real system has one. Here: how is the feed built?</p>
...
<p><b>5. Walk the failure & growth paths.</b> What breaks first? The feed store → cache hot feeds. Celebrity posts → the hybrid. Image bandwidth → CDN already took it. Then say what you are NOT building: no stories, no ranking algorithm in v1. Chronological ships first.</p>
<p>What the method did: numbers chose read-vs-write focus, the API forced pagination thinking, the data model surfaced object storage, and ONE decision (fan-out) got the real analysis. Depth on the decision that matters beats shallow coverage of ten boxes, in interviews and in the design review at work.</p>`,
...
<p>A design that lives in your head scales to one person and zero months. Senior engineers <b>decide in writing</b>. This is the cheapest known way to find flaws before building, get real review, and let 2027-you understand why 2026-you did this.</p>
...
<p><b>The ADR</b> (Architecture Decision Record) is the design doc's small sibling. Half a page recording ONE decision, numbered and immutable, living in the repo (<code>docs/adr/0007-use-postgres-for-progress.md</code>). Its power is the <b>Context → Decision → Consequences</b> discipline. Consequences <i>include the bad ones you accept</i>:</p>
...
<p>Why writing beats meetings. Text is reviewable asynchronously and by more people. Writing exposes hand-waving that speech hides ("...and then somehow the cache invalidates"). The alternatives section forces real comparison. The archive turns "why on earth is it built this way?" from archaeology into a link. Every "why do we even have this queue?" argument that ends in a shrug is a missing ADR.</p>`,
