...
tests:[{d:'Two pointers from both ends',re:'\\w+\\s*=\\s*0[\\s\\S]*?\\w+\\s*=\\s*s\\.length\\s*\\(\\s*\\)\\s*-\\s*1'},{d:'Skips non-alphanumerics',re:'Character\\.isLetterOrDigit'},{d:'Case-insensitive compare, with a plain false on mismatch',re:'(?=[\\s\\S]*Character\\.toLowerCase)(?=[\\s\\S]*return\\s+false\\s*;)'},{d:'No cleaned-string allocation',re:'replaceAll|toCharArray\\(\\)\\.length|StringBuilder',not:true}],
...
tests:[{d:'Method signature',re:'static\\s+boolean\\s+isPrime\\s*\\(\\s*int\\s+n\\s*\\)'},{d:'n < 2 returns false',re:'n\\s*<\\s*2\\s*\\)?\\s*\\{?\\s*return\\s+false\\s*;'},{d:'Loops only to sqrt',re:'\\w+\\s*\\*\\s*\\w+\\s*<=\\s*n'},{d:'Divisibility test',re:'%\\s*\\w+\\s*==\\s*0'}],
...
tests:[{d:'Method signature',re:'static\\s+boolean\\s+hasDuplicate\\s*\\(\\s*int\\[\\]\\s+a\\s*\\)'},{d:'Uses a HashSet',re:'new\\s+HashSet<'},{d:'A rejected add (or a contains hit) returns true',re:'!\\s*\\w+\\.add\\s*\\([^;]{0,40}\\)\\s*\\{?\\s*return\\s+true\\s*;|\\.contains\\s*\\([^;]{0,40}\\)\\s*\\{?\\s*return\\s+true\\s*;'}],
...
tests:[{d:'Method signature',re:'static\\s+boolean\\s+isAnagram\\s*\\(\\s*String\\s+a\\s*,\\s*String\\s+b\\s*\\)'},{d:'Different lengths return false fast',re:'length\\s*\\(\\s*\\)\\s*!=\\s*\\w+\\.length\\s*\\(\\s*\\)\\s*\\)\\s*\\{?\\s*return\\s+false\\s*;'},{d:'Count array of 26',re:'new\\s+int\\[\\s*26\\s*\\]'}],
...
tests:[{d:'Void in-place signature',re:'static\\s+void\\s+rotate\\s*\\(\\s*int\\[\\]\\s+nums\\s*,\\s*int\\s+k\\s*\\)'},{d:'Normalizes k',re:'k\\s*%=\\s*\\w+'},{d:'Uses a reverse helper three times',re:'rotate\\s*\\([^)]*\\)\\s*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?reverse\\s*\\((?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?reverse\\s*\\((?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?reverse\\s*\\('}],
...
tests:[{d:'Method signature',re:'static\\s+int\\s+minSubArrayLen\\s*\\(\\s*int\\s+target\\s*,\\s*int\\[\\]\\s+nums\\s*\\)'},{d:'Shrinks while sum meets target',re:'while\\s*\\(\\s*sum\\s*>=\\s*target\\s*\\)'},{d:'Tracks minimum length and returns a length',re:'(?=[\\s\\S]*Math\\.min)(?=[\\s\\S]*return\\s+(?!\\s*!)[^;]{0,100}(best|0)\\b)'},{d:'Removes from the left',re:'sum\\s*-=\\s*nums\\[\\s*\\w+'}],
...
tests:[{d:'Method signature',re:'static\\s+boolean\\s+searchMatrix\\s*\\(\\s*int\\[\\]\\[\\]\\s+m\\s*,\\s*int\\s+target\\s*\\)'},{d:'Binary search loop that returns true on a hit',re:'(?=[\\s\\S]*while\\s*\\(\\s*lo\\s*<=\\s*hi\\s*\\))(?=[\\s\\S]*(==\\s*target|target\\s*==)[^;]{0,40}return\\s+true\\s*;)'},{d:'Maps flat index to row/col',re:'/\\s*cols[\\s\\S]*?%\\s*cols|%\\s*cols[\\s\\S]*?/\\s*cols'}],
...
tests:[{d:'Doubly linked removal (both directions relinked)',re:'prev\\.next\\s*=\\s*\\w+\\.next(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?next\\.prev\\s*=\\s*\\w+\\.prev|n\\.prev\\.next\\s*=\\s*n\\.next'},{d:'Insertion at the front behind the head sentinel',re:'head\\.next(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?head\\.next\\s*='},{d:'get refreshes recency (remove + addFront)',re:'int\\s+get\\s*\\([^)]*\\)\\s*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?remove\\s*\\((?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?addFront\\s*\\('},{d:'Eviction from tail.prev with map cleanup',re:'tail\\.prev(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?map\\.remove\\s*\\('},{d:'Capacity check drives eviction',re:'size\\s*\\(\\s*\\)\\s*>\\s*capacity|map\\.size\\s*\\(\\s*\\)\\s*>\\s*capacity'}],
...
tests:[{d:'Method signature',re:'static\\s+String\\s+minWindow\\s*\\(\\s*String\\s+s\\s*,\\s*String\\s+t\\s*\\)'},{d:'Char need table',re:'new\\s+int\\[\\s*128\\s*\\]'},{d:'Required counter drives shrink',re:'required'},{d:'The substring is what comes back',re:'return\\s+(?!\\s*!)[^;]{0,120}substring\\s*\\('}],
...
tests:[{d:'Method signature',re:'static\\s+boolean\\s+wordBreak\\s*\\(\\s*String\\s+s\\s*,\\s*List<String>\\s+dict\\s*\\)'},{d:'Dictionary as a Set',re:'new\\s+HashSet<'},{d:'Boolean DP array with dp[0] true, and the last cell is the answer',re:'(?=[\\s\\S]*dp\\[\\s*0\\s*\\]\\s*=\\s*true)(?=[\\s\\S]*return\\s+(?!\\s*!)[^;]{0,60}dp\\s*\\[)'},{d:'Checks a substring against the set',re:'contains\\s*\\(\\s*s\\.substring'}],
...
tests:[{d:'Method signature',re:'static\\s+boolean\\s+canFinish\\s*\\(\\s*int\\s+numCourses\\s*,\\s*int\\[\\]\\[\\]\\s+prereq\\s*\\)'},{d:'Indegree array',re:'int\\[\\]\\s+indeg'},{d:'Queue-based BFS',re:'ArrayDeque<'},{d:'Returns whether the processed count reached numCourses',re:'return\\s+(?!\\s*!)[^;]{0,60}==\\s*numCourses|return\\s+(?!\\s*!)[^;]{0,60}numCourses\\s*=='}],
