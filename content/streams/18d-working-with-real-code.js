...
tests:[{d:'Four @Test methods present',re:'@Test[\\s\\S]*?@Test[\\s\\S]*?@Test[\\s\\S]*?@Test'},{d:'Plain tier pinned at 30.0',re:'assertEquals\\s*\\(\\s*30\\.0\\s*,[\\s\\S]*?fee\\s*\\(\\s*3\\s*,\\s*"basic"\\s*\\)'},{d:'Gold discount pinned at 25.0',re:'assertEquals\\s*\\(\\s*25\\.0\\s*,[\\s\\S]*?fee\\s*\\(\\s*3\\s*,\\s*"gold"\\s*\\)'},{d:'The weird zero-qty behavior is pinned, not fixed',re:'zeroQty_pinned[\\s\\S]*?assertEquals\\s*\\(\\s*0\\.0\\s*,[\\s\\S]*?fee\\s*\\(\\s*0\\s*,'},{d:'Comment marks the pin as deliberate',raw:true,re:'//\\s*pinned:\\s*callers\\s+rely\\s+on\\s+this'},{d:'Every double assertion carries a delta',re:'assertEquals\\s*\\(\\s*[\\d.]+\\s*,[\\s\\S]{0,80}?,\\s*0\\.001\\s*\\)'}],
