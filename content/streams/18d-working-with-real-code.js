...
<p><b>When reviewing, look for things in severity order, and label them:</b></p>
<ul>
...
<p><b>After: the blameless postmortem.</b> Everything rests on its premise: <i>the engineer acted reasonably given what they knew; the SYSTEM let a reasonable action cause damage</i>, so fix the system. Name a human cause and people hide the next near-miss; name a system cause and you get action items. The document: timeline (detection → mitigation → resolution, with timestamps), impact quantified plainly, <b>root causes plural</b> (there are always several: the bug AND the review that missed it AND the missing alert AND the slow rollback), and action items that are <i>specific, owned, and deadlined</i> ("add an alert on p99 &gt; 500ms, owner: you, by Friday"), never "be more careful", which is a wish wearing a checkbox.</p>
<p>The quiet payoff: teams that run real postmortems convert every outage into infrastructure. Teams that don't, meet the same outage annually like a holiday.</p>`,
