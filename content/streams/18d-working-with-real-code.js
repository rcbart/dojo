...
<p>Seniors spend far more time reading code than writing it. Reading a 500k-line codebase is a learned skill with a method, not an act of patience.</p>
<ul>
<li><b>Start from the entry points, not the file tree.</b> Where does execution begin: <code>main</code>, the controllers, the message listeners, the scheduled jobs? Reading alphabetically by folder is how you learn nothing in four hours.</li>
<li><b>Trace one request end to end.</b> Pick one concrete operation ("what happens when a user posts a comment?") and follow it through every layer. Write the chain down as you go: <code>CommentController → CommentService → SpamChecker → CommentRepo → events</code>. One completed trace teaches more architecture than any diagram in the wiki. The wiki is outdated anyway: trust the code, verify the docs.</li>
<li><b>Interrogate the history.</b> Git is a time machine with testimony. Run <code>git log --follow</code> on a confusing file. <code>git log -S "methodName"</code> (the "pickaxe") finds every commit that ever touched a string: who introduced this, with what message, alongside what else? <code>git blame</code> answers "why is this line weird" more often than the line itself does. A strange <code>if</code> guarding nothing visible, plus a blame pointing at "fix prod incident 2023-04", is weirdness that is holding something up. Leave it until you understand it.</li>
<li><b>Read the tests as documentation.</b> Test names are executable claims about intended behavior, often the only spec in the building that runs. If there are no tests, the archaeology just told you the most important fact about this code.</li>
<li><b>Build a map, not a memory.</b> Keep notes: entry points found, the trace chains, surprises, questions. Your PRs will be reviewed by people who hold this map in their heads. The notes are how you catch up in weeks instead of years.</li>
</ul>
...
<p>Unfamiliar code is <i>evidence</i>, not mess. Every weird line was written by someone with a reason under some constraint. Archaeology recovers the reason before you delete the constraint: that is the next lesson's fence.</p>`,
...
prompt:`"You join a team with a 400k-line service, no docs, the author left. Walk me through your first week." This is the real senior interview question. One per numbered line: (1) start reading from the file tree alphabetically, or from the ____ ____ where execution begins (two words); (2) the single most instructive exercise: ____ one request end to end (one word, a verb); (3) to find who introduced a mysterious method and why, the git command family (one word: the flag letter tool, "-S", is the ____); (4) the executable spec of intended behavior, when it exists (one word); (5) you find a weird guard clause; git blame says "hotfix prod incident": delete it or ____ ____ first (two words); (6) what you produce for yourself as you go, so week 3 isn't week 1 again (one word: notes/a ____).`,
starter:`1.
2.
3.
4.
...
6. map
`,
tests:[{d:'Q1: start from entry points',re:'1\\.\\s*entry\\s+points',flags:'is'},{d:'Q2: trace a request end to end',re:'2\\.\\s*trace',flags:'is'},{d:'Q3: the pickaxe (git log -S)',re:'3\\.\\s*pickaxe',flags:'is'},{d:'Q4: tests are the executable spec',re:'4\\.\\s*tests',flags:'is'},{d:'Q5: understand before deleting (Chesterton)',re:'5\\.\\s*understand',flags:'is'},{d:'Q6: build a map, not a memory',re:'6\\.\\s*map',flags:'is'}],
...
<p>The scariest sentence in professional software: <i>"just change this one thing"</i>, in a class with no tests, written by someone who left, that production has depended on for five years. The legacy-code discipline starts with a redefinition. Michael Feathers' book is the canon: <b>legacy code is code without tests</b>, age irrelevant. Its first move is never the change itself.</p>
<p><b>Characterization tests</b> pin down what the code <i>currently does</i>, not what it should do. Write a test with a guessed assertion, run it, and let the CODE tell you the real answer. Then assert <i>that</i>, weirdness included:</p>
...
<p>Pin bugs rather than fixing them on sight, because <b>current behavior is what production depends on</b>. Some caller somewhere may rely on zone "Q" returning 0 instead of throwing. Characterize first, as the safety net. Change second, one thing at a time. Fix the bug as its own deliberate, visible commit, not as a silent side effect of a refactor.</p>
<p>The blocker you will hit immediately: the legacy class news up its own dependencies. <code>new SmtpMailer()</code> inside the method is your DI lessons' anti-pattern, live and in the wild, so testing it sends email. <b>DI</b> is dependency injection: an object is handed the things it needs (a mailer, a clock) instead of building them itself, which is what makes a class testable on its own. This class builds its own, so nothing can be swapped. The escape is a <b>seam</b>: the smallest change that makes a dependency swappable <i>without changing behavior</i>. Extract the <code>new</code> into a constructor parameter, keeping a default constructor that delegates with the old value so no caller breaks. Or extract a protected method a test subclass can override. Seam first, characterize second. Only then, with the net under you, change the thing they sent you in to change.</p>`,
...
tests:[{d:'Four @Test methods present',re:'@Test[\\s\\S]*?@Test[\\s\\S]*?@Test[\\s\\S]*?@Test'},{d:'Plain tier pinned at 30.0',re:'assertEquals\\s*\\(\\s*30\\.0\\s*,(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?fee\\s*\\(\\s*3\\s*,\\s*"basic"\\s*\\)'},{d:'Gold discount pinned at 25.0',re:'assertEquals\\s*\\(\\s*25\\.0\\s*,(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?fee\\s*\\(\\s*3\\s*,\\s*"gold"\\s*\\)'},{d:'The weird zero-qty behavior is pinned, not fixed',re:'zeroQty_pinned\\s*\\([^)]*\\)\\s*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?assertEquals\\s*\\(\\s*0\\.0\\s*,(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?fee\\s*\\(\\s*0\\s*,'},{d:'Comment marks the pin as deliberate',raw:true,re:'//\\s*pinned:\\s*callers\\s+rely\\s+on\\s+this'},{d:'Every double assertion carries a delta',re:'assertEquals\\s*\\(\\s*[\\d.]+\\s*,[\\s\\S]{0,80}?,\\s*0\\.001\\s*\\)'}],
...
tests:[{d:'Five @Test methods',re:'@Test[\\s\\S]*?@Test[\\s\\S]*?@Test[\\s\\S]*?@Test[\\s\\S]*?@Test'},{d:'Normal A grade pinned',re:'assertEquals\\s*\\(\\s*"A"\\s*,(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?grade\\s*\\(\\s*95\\s*\\)'},{d:'The no-D quirk pinned with a marker comment',raw:true,re:'noDgrade_pinned\\s*\\([^)]*\\)\\s*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?//\\s*pinned(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?assertEquals\\s*\\(\\s*"F"\\s*,(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?grade\\s*\\(\\s*65'},{d:'Over-100 behavior pinned (not "fixed" to an error)',re:'over100_pinned\\s*\\([^)]*\\)\\s*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?assertEquals\\s*\\(\\s*"A"\\s*,(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?grade\\s*\\(\\s*150'},{d:'Negative-score behavior pinned',re:'negative_pinned\\s*\\([^)]*\\)\\s*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?assertEquals\\s*\\(\\s*"F"\\s*,(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?grade\\s*\\(\\s*-5'},{d:'Boundary at 80 pinned to B',re:'grade\\s*\\(\\s*80\\s*\\)(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?"B"|assertEquals\\s*\\(\\s*"B"\\s*,(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?grade\\s*\\(\\s*80'}],
...
hints:['Pin what IS, not what SHOULD BE; the quirks are holding something up until proven otherwise.','The _pinned naming + comment is communication: stop the next dev from "helpfully" deleting the weird test.','Boundary values (80, 90, 70) are where grading bugs hide; pin at least one.']},
...
<p>With behavior pinned, you may change structure. What separates refactoring from rewriting is <b>step size</b>. Fowler's definition: a refactoring <i>improves structure while preserving behavior</i>, in tiny steps with the tests green between every one.</p>
<ul>
<li><b>The rhythm</b>: change one small thing → run the tests → commit or revert. A failing step is <i>discarded</i>, never debugged: it took 3 minutes, and that cheap revert is the whole case for small steps. Two hats, worn separately. The refactoring hat never changes behavior, the feature hat never restructures. One hat per commit.</li>
<li><b>Six named moves</b> cover 90% of daily work. <i>Extract Method</i>: name a block, the workhorse. <i>Rename</i>: the highest value per risk in software. <i>Extract Class</i>: a class hoarding two jobs (SRP). <i>Inline</i>: dissolve needless indirection. <i>Replace Conditional with Polymorphism</i>: your Shipping enum in the wild. <i>Introduce Parameter Object</i>: four booleans marching in formation are a type begging to exist.</li>
<li><b>Smells tell you which move.</b> A comment saying "// now compute the surcharge" is Extract Method in disguise, with the comment as the method name. A 40-line method wants dissection. Three arguments traveling together want an object. A switch on type in four places wants polymorphism.</li>
<li><b>The strangler fig</b> is the same discipline at architecture scale (your launch plan, your Judge0-first runner). Stand the new implementation beside the old and route traffic over incrementally: one endpoint, one percent, one tenant. Keep the old path warm for instant retreat. Delete it only when the new one has taken all traffic and held. A rewrite from scratch with one cutover is the most reliably fatal project shape in the industry. The fig survives because every intermediate state ships and works.</li>
</ul>
...
<p>The senior tell in a diff: fifteen boring green commits like "extract TariffCalculator from Billing (no behavior change)", versus one commit titled "refactored billing" touching 40 files. The first is reviewable, bisectable and revertible. The second is a prayer.</p>`,
...
<p>Code review is where a team's engineering culture is decided. Reviewing well is a distinct skill from coding well.</p>
<p><b>When reviewing, look in severity order and label what you find:</b></p>
<ul>
<li><b>1. Correctness</b>: does it do what it claims? Edge cases (null, empty, concurrent, huge), off-by-ones, leaked resources, race windows: your whole dojo as a checklist.</li>
<li><b>2. Security & data</b>: input validated at the boundary? Ownership checked (the IDOR lesson)? <b>IDOR</b> is insecure direct object reference: the application checks that you are logged in, then hands over whatever record you name in the URL without checking it is yours. Change <code>/orders/1042</code> to <code>/orders/1043</code> and you are reading someone else's order. Secrets out of code? Migrations reversible?</li>
<li><b>3. Design</b>: right place for this logic? Coupling introduced? Will this shape survive the next three features? Do the tests test <i>behavior</i>, not mocks?</li>
<li><b>4. Readability</b>: naming, dead code, comment drift.</li>
<li><b>5. Style: automate it.</b> If humans are debating brace position, the team lacks a formatter, not opinions. Review time spent on style is stolen from correctness.</li>
</ul>
<p><b>Phrasing is engineering too.</b> Comment on the <i>code, never the author</i>: "this method re-reads the file per loop iteration", not "you always do this". <b>Ask before asserting</b>: "what happens here when the list is empty?" beats "this breaks on empty", especially when you are the one who is wrong. Label severity (<code>blocking:</code> vs <code>suggestion:</code> vs <code>nit:</code>) so the author knows what must change and what is offered. And <b>say what is good</b>: "this test names the bug it prevents, nice" teaches as much as a critique and costs one line.</p>
<p><b>Receiving is the half seniors model.</b> The review is of the code, not of you. Every question is a place the code failed to explain itself. The reviewer is the first confused reader of many, so fix the code or add the why-comment. An answer in the thread evaporates. Gratitude for a hard review is a senior tell: someone spent an hour improving your change before production reviewed it the expensive way.</p>
<p><b>The author's duty comes first</b>: small PRs. Review quality collapses with size. A 200-line PR gets its bugs found. A 2,000-line PR gets "LGTM", approval by exhaustion. Stacking small PRs improves your reviews more than any other habit. The fifteen boring commits from the refactoring lesson are also what a reviewable change looks like.</p>`,
...
behavior:`1. The two real dangers carry the blocking: label (severity is information), and both name the fix, not just the crime. 2. The null-contract issue arrives as a genuine question: the reviewer might be missing context, and the question teaches either way. 3. The naming point is labeled nit:, so the author knows it won't block merge. 4. Every comment discusses the code's behavior; none discusses the author. That's the difference between a review culture people learn from and one they armor against. 5. Bonus you'd add in real life: one line about anything done well.`,
...
`,
tests:[{d:'Blocking: names the atomicity/race bug + AtomicInteger fix',re:'1\\.\\s*blocking:(?:(?!\\n\\s*2\\.)[\\s\\S])*?(atomic|race)(?:(?!\\n\\s*2\\.)[\\s\\S])*?AtomicInteger',flags:'i'},{d:'Question about uniqueness across restarts, with a "?"',re:'2\\.\\s*question:(?:(?!\\n\\s*3\\.)[\\s\\S])*?restart(?:(?!\\n\\s*3\\.)[\\s\\S])*?\\?',flags:'i'},{d:'Nit offers a concrete better name',re:'3\\.\\s*(nit|suggestion):[\\s\\S]*?nextId',flags:'i'},{d:'Comments address the code, not the author',re:'you\\s+(always|never)',not:true,flags:'i'}],
behavior:`1. The blocking comment names the exact defect (read-modify-write race) and the exact fix (AtomicInteger.getAndIncrement): symptom, consequence, remedy, the shape of every good blocking note. 2. The restart question raises a design issue the diff can't answer: an in-memory counter resets to 0 on deploy, silently colliding with old ids; it is asked as a question because the reviewer might lack context, and it is instructive either way. 3. The naming point is tagged nit so the author knows it won't block. 4. Not one comment attacks the person. That's the difference between a review culture engineers learn from and one they armor against, and it's a graded senior skill at every company that reviews well.`,
...
<p>Production is down. What you do in the next ten minutes, and the next ten days, is a senior skill almost nobody teaches.</p>
<p><b>During: mitigate first, diagnose second.</b> The instinct to find the bug first is wrong: the user-facing clock is running. The triage sequence:</p>
<ul>
<li><b>1. What changed?</b> Ninety percent of incidents follow a change. Last deploy? Feature flag? Config push? A dependency's status page? <code>git log --since</code> and the deploy dashboard are your first minute.</li>
<li><b>2. Can we roll back or turn it off?</b> Roll back the suspect deploy, flip the flag off, move traffic away. <i>Mitigation restores users without requiring understanding.</i> Fix-forward under pressure is plan B, chosen deliberately, not by ego.</li>
<li><b>3. Only then, diagnose</b>, with the telemetry you built. Logs with MDC request-ids: trace ONE failing request end to end. <b>MDC</b> is the mapped diagnostic context: a per-thread map (request id, user) that your logging framework prints on every line from that request, so one grep pulls out the whole story. Metrics for the shape: all requests or one endpoint? Gradual or cliff? A cliff points at a change, a ramp at a resource leak. Thread dumps when the service is alive but frozen: two dumps 10 seconds apart, and the threads that did not move are your suspects. Dozens parked in <code>WAITING</code> on the same pool means a starved pool.</li>
...
<p><b>After: the blameless postmortem.</b> Its premise: <i>the engineer acted reasonably given what they knew, and the SYSTEM let a reasonable action cause damage</i>. So fix the system. Name a human cause and people hide the next near-miss. Name a system cause and you get action items. The document: a timeline (detection → mitigation → resolution, with timestamps), impact quantified plainly, <b>root causes plural</b> (the bug AND the review that missed it AND the missing alert AND the slow rollback), and action items that are <i>specific, owned, and deadlined</i>: "add an alert on p99 &gt; 500ms, owner: you, by Friday". Never "be more careful": a wish wearing a checkbox.</p>
<p>Teams that run real postmortems convert every outage into infrastructure. Teams that do not meet the same outage annually, like a holiday.</p>`,
