...
<p>Two readings to internalize before moving on: <code>List&lt;String&gt;</code> — "a List of Strings"; <code>Map&lt;String, Integer&gt;</code> — "a Map from Strings to Integers" (a phone book: name → number). The angle brackets always answer the question <i>"of what?"</i></p>`,
...
behavior:`1. total(List.of(1, 2.5)) == 3.5 — works for any Number subtype list. 2. pad(list, 3) adds 1, 2, 3 to a List<Number> or List<Object> target. 3. biggest(List.of(3, 9, 2)) == 9; biggest(List.of("b","a")) equals "b". 4. Signatures use the exact wildcard forms — swap extends/super and the AI runner will flag it.`,
...
behavior:`1. same — one List class serves every label; getClass() proves it. 2. type erasure — the compiler checks, then strips. 3. no — T is Object at runtime; pass a Class<T> or Supplier<T> when construction is needed. 4. List<?> — "some list", the only honest runtime question. 5. raw — the pre-2004 compatibility mode, never for new code. 6. unchecked cast — the compiler admitting it cannot back your assertion.`,
