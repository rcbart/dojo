...
<p>You are on a call. Something is slow. Someone pastes a link to 40 million log lines. Opening it in an editor fails on the first step: the editor loads the whole file into memory, and the file is bigger than your laptop's memory. The command line asks a different question: <b>"what transformation turns this file into the answer?"</b></p>
<p>The model has three parts.</p>
<ul>
<li><b>A program is a filter.</b> It reads bytes from <b>stdin</b>, writes results to <b>stdout</b>, and writes complaints to <b>stderr</b>, a separate channel so diagnostics never contaminate data. <code>grep</code>, <code>sed</code>, <code>awk</code>, <code>jq</code>, <code>sort</code>, <code>uniq</code>, <code>head</code> all have the same shape: text in, text out.</li>
<li><b>The pipe connects one filter's stdout to the next one's stdin.</b> <code>a | b</code> does not mean "run a, then run b". Both processes start at once and data flows through as it is produced. Nothing is ever fully in memory. <code>grep ERROR huge.log | head -5</code> answers in milliseconds on a 40GB file: <code>head</code> exits after five lines, the kernel sends <code>SIGPIPE</code> to <code>grep</code>, and <code>grep</code> stops reading.</li>
<li><b>Every program returns an exit code.</b> <code>0</code> means success, non-zero means failure. <code>grep</code> returns <code>0</code> if it matched, <code>1</code> if it matched nothing, <code>2</code> on a real error. <code>if grep -q ERROR app.log; then ...</code> works because the search result <i>is</i> the condition. It is also a classic CI trap: under <code>set -e</code>, a <code>grep</code> that finds nothing aborts the script.</li>
</ul>
...
<p>The last line matters. By default a pipeline reports only the exit status of its final command, so <code>badcommand | sort | head</code> succeeds with status <code>0</code> while producing nothing. In a deploy script that is a silent failure. <code>set -o pipefail</code> makes the pipeline fail if any stage failed.</p>
<p><b>Why this design outlived its machines.</b> Doug McIlroy's 1964 memo asked for a way to "screw programs together like garden hose". It still works because the tools agreed on the narrowest possible interface: <b>lines of text</b>. No shared library, no schema, no version negotiation. <code>jq</code> was written forty years after <code>sed</code> and composes with it, because neither knows the other exists. Text is a poor data model, and it is the only one everything can agree on.</p>
...
docs:[['POSIX: shell command language','https://pubs.opengroup.org/onlinepubs/9699919799/utilities/V3_chap02.html'],['McIlroy on the origin of pipes','https://www.nokia.com/bell-labs/about/dennis-m-ritchie/mdmpipe.html'],['GNU coreutils manual','https://www.gnu.org/software/coreutils/manual/coreutils.html']],
...
<p>Nearly everyone can use <code>grep</code> badly. Used well it answers three questions: <i>does this appear?</i>, <i>how often?</i>, <i>what surrounds it?</i> The difference is six flags and one failure mode.</p>
<ul>
<li><b><code>-F</code>, fixed strings.</b> Your pattern is <i>data</i>, not a regex: <code>grep -F '/api/v1.0/orders'</code> treats the dot as a dot. Without <code>-F</code> that dot matches any character, so the pattern also matches <code>/api/v1X0/orders</code>. <code>-F</code> is faster, since no regex engine runs. It is also the only safe choice when the pattern comes from a variable, a file or a user: a stray <code>.</code>, <code>*</code> or <code>[</code> otherwise changes the meaning or errors out.</li>
<li><b><code>-E</code>, extended regex.</b> Use it for <code>|</code>, <code>+</code>, <code>?</code>, <code>{2,4}</code> or groups without backslashes. Basic <code>grep</code> requires <code>\\|</code> and <code>\\+</code>, and the patterns end up unreadable. The rule: <b>-F if you can, -E if you must, plain grep almost never.</b></li>
<li><b><code>-o</code>, print only the match.</b> grep becomes an <i>extractor</i>: <code>grep -oE 'trace=[0-9a-f]+'</code> emits just the trace ids, one per line, ready for <code>sort | uniq -c</code>.</li>
...
<p><b>The mistake that costs you an hour.</b> <code>grep 200 app.log</code> looks like "find the successful requests". It matches the status field <code>200</code>, and also <code>1200ms</code>, <code>trace=2004ab</code>, and the timestamp <code>...T12:00:20Z</code>. On the sample log it reports 911 lines where 910 have status 200. On your log the error will be larger. A pattern with no anchors matches <b>anywhere on the line</b>, and a log line is full of numbers.</p>
<p>The fixes, in order of preference. <b>Match the field with its delimiters</b>: <code>grep -F ' 200 '</code>. <b>Anchor to a position</b>: <code>^</code> for start of line, <code>$</code> for end, <code>\\b</code> for a word boundary. When the field's position matters, stop using grep and use <code>awk '$6==200'</code>, which compares the sixth field and cannot match anything else. Two more traps in the same family. <code>-w</code> matches whole words but treats <code>/api/orders</code> as several words. Case matters unless you pass <code>-i</code>: <code>error</code> and <code>ERROR</code> are different searches.</p>`,
...
tests:[{d:'the line is split on whitespace',re:'split\\s*\\(\\s*/'},{d:'runs of whitespace are one separator',re:'\\\\s\\+'},{d:'the field is selected by index',re:'\\[index\\]'},{d:'the exact comparison is what the function returns',re:'return\\s+(?!\\s*!\\s*\\()[^;]{0,120}==='}],
...
<p>Two conventions do most of the work. <b><code>-n</code> suppresses the automatic print</b>, so <code>sed -n '/x/p'</code> means "print nothing except what I ask for". Without <code>-n</code> you get every line, plus a second copy of the matches. <b><code>$</code> means the last line</b>, so <code>/start/,$</code> is "from the first match to the end", the everyday "show me everything after the deploy" address.</p>
<p><b>Substitution</b> is one command among many: <code>s/pattern/replacement/flags</code>. Three flags matter. <code>g</code> replaces every occurrence on the line, not just the first. <code>i</code> is case-insensitive. <code>p</code> prints the result and pairs with <code>-n</code>. The delimiter is not special. When the pattern contains slashes, switch to <code>s#...#...#</code> or <code>s|...|...|</code> and stop escaping. Use <code>-E</code> for extended regex so groups are <code>(...)</code> rather than <code>\\(...\\)</code>, and refer back to them as <code>\\1</code>, <code>\\2</code>.</p>
...
<p>The middle line is the idiom to memorize: <b><code>-n</code> plus <code>s///p</code> is extraction</b>. Match the line, keep only the captured group, print that. The third makes traffic countable: collapse <code>/api/orders/1234</code> and <code>/api/orders/9876</code> into one normalized route, and <code>sort | uniq -c</code> tells you something true.</p>
<p><b>Where sed stops being the right tool.</b> Three places. <b>When you need fields</b>: "the sixth column" is awk's job, and in sed it means counting spaces in a regex. <b>When you need arithmetic or state across lines</b>: sums, averages, grouping. sed has no numbers. <b>When the input is structured</b>: HTML, JSON, CSV with quoted commas. A regex cannot parse nested structure, and the log line that breaks your pattern will be the one you needed.</p>
<p>Two operational warnings. <code>.*</code> is <b>greedy</b>: it matches as much as possible, so <code>s/.*=//</code> deletes through the <i>last</i> <code>=</code> on the line, not the first. That is usually the bug in an extraction that returns almost the right thing. <code>-i</code> edits files <b>in place</b>, with no undo. GNU sed takes <code>-i</code> alone, BSD/macOS sed requires an argument (<code>-i ''</code>), so the same command does different things on a Mac and in CI. Run it without <code>-i</code> first and read the output.</p>`,
...
<p>grep selects lines. sed rewrites them. Neither can answer <i>"what is the average latency per endpoint?"</i>, because neither can add. <b>awk</b> is a small programming language that has been in every Unix since 1977, and its whole grammar is one line:</p>
<div class="codeSample">awk 'PATTERN { ACTION }' file</div>
<p>For each line, if <code>PATTERN</code> is true, run <code>ACTION</code>. Omit the pattern and the action runs on every line. Omit the action and matching lines are printed. So <code>awk '$6==500'</code> is a complete program (pattern only), and <code>awk '{print $5}'</code> is too.</p>
<p><b>Fields come free.</b> awk splits every line on whitespace before you see it: <code>$1</code>, <code>$2</code>, … <code>$NF</code> (the last field), with <code>NF</code> the field count and <code>NR</code> the line number. <code>$0</code> is the whole line. <code>-F,</code> switches the separator to a comma, <code>-F'\\t'</code> to a tab. "The third column" or "the last column" took a page of regex in sed and is now two characters.</p>
...
<p><b>BEGIN and END</b> are patterns that match before the first line and after the last. Headers and totals go there. Variables need no declaration and start at <code>0</code> or <code>""</code>, so <code>{s+=$7; n++} END {print s/n}</code> is a complete average. awk coerces strings to numbers by reading the leading digits: <code>"87ms"+0</code> is <b>87</b>. Durations with units are a problem everywhere else and not here.</p>
<p><b>The part that beats every other tool: associative arrays.</b> An awk array is a hash map with string keys. It turns "group by" into one line: one pass over the data, no sorting, memory proportional to the number of <i>distinct keys</i> rather than to the file:</p>
...
<p>That is <code>SELECT route, avg(dur) FROM log GROUP BY route</code> with no database, no import and no schema, on a file you received ninety seconds ago. The <b>aggregation</b>, not the syntax, is the reason awk is still worth learning.</p>
<p><b>Where it stops.</b> Whitespace splitting breaks on fields that contain spaces or quoted separators. A real CSV parser is not a <code>-F,</code>. Sorting inside awk is <code>gawk</code>-only (<code>asort</code>), so portable scripts pipe to <code>sort</code> instead. Once a script grows past a screen, or needs joins, dates or JSON, it wants to be Python. Note the number above, though: checkout's <b>average</b> is 76.3ms. Its p99 is nearly a second. Averages hide the failure you are being paged about. That is the capstone.</p>`,
...
<p>Every tool so far treats a log line as text and re-derives its structure with a regex. Modern services emit <b>one JSON object per line</b>, so the structure is already there. JS Dojo's structured-logging lesson makes the same case: log to stdout and let the platform collect it. The cost is that <code>awk '{print $5}'</code> becomes meaningless. Fields have names, not positions, they may be absent, and a message can contain a comma, a quote or a newline without breaking anything. <b><code>jq</code> is awk for JSON</b>: a filter language over structured values.</p>
...
<ul>
<li><b>Paths select.</b> <code>jq '.route'</code> pulls one field, <code>.user.id</code> reaches into nesting, <code>.[]</code> iterates an array. <code>-r</code> prints strings <i>raw</i>. Without it every value comes back quoted, and the quotes poison your <code>sort | uniq -c</code>.</li>
<li><b><code>select()</code> filters.</b> It is the <code>WHERE</code> clause: <code>select(.status &gt;= 500)</code>, <code>select(.level=="error" and .dur_ms &gt; 60)</code>. Comparisons are typed. <code>.status &gt;= 500</code> is a number comparison, not a string one, so the over-matching bug from the grep lesson cannot happen.</li>
...
<p>Two survival details. <b><code>//</code> supplies a default</b> when a field is missing or null (<code>.user.id // "anonymous"</code>). Real logs are ragged, and a missing key otherwise yields <code>null</code> that spreads through the rest of your expression. <b><code>--arg</code> passes shell values in as data</b>, never by string-splicing them into the program, for the same reason <code>awk -v</code> and SQL parameters exist.</p>
<p><b>The real limits.</b> One malformed line aborts the run with <code>parse error</code>, so mixed text-and-JSON logs need <code>grep '^{'</code> in front, or jq's <code>--seq</code>/<code>-R 'fromjson?'</code> to skip the wreckage. <code>-s</code> is not streaming. The field names are a contract nobody enforces. Rename <code>dur_ms</code> to <code>duration_ms</code> in a service and every dashboard and pipeline that read it silently returns <code>null</code>. That is the structured-logging equivalent of a schema migration, so the field list belongs in a review checklist.</p>`,
...
<p>The page arrives at 15:40 on Tuesday: <i>"checkout feels slow since the deploy."</i> You have <code>app.log</code>: 990 lines, one request each, and a deploy that went out at <b>14:00</b>. No dashboard, no tracing UI, no time to build one. The question: <b>which route's p99 latency got worse after 14:00, and by how much?</b></p>
<p><b>Step 1, do not start with the average.</b> The previous lesson showed why. Checkout's mean after the deploy is 86.1ms against 76.0 for orders: up a third on 134 samples, which reads as noise. Compute means and you will close the incident while checkout keeps timing out for one user in thirty. A mean describes the typical request. An outage lives in the tail. You want a percentile.</p>
...
<div class="codeSample">awk '$1 &gt;= "2026-08-11T14:00:00Z" {print $5, $7+0}' app.log \\
  ...
  | sort</div>
<p>Run it for the window <b>before</b> 14:00 and then for the window after:</p>
...
<p><b>Step 3, read what the numbers say, and what they do not.</b> One route regressed, and only in the tail. Checkout's <i>median</i> after the deploy is 67ms, healthy. So roughly 1–3% of checkout requests now take about a second while the rest are unaffected. That is the signature of a dependency that is slow on a subset of calls, not of a service that is uniformly slower: a cold cache, one bad shard, an added call on a branch only some requests take. It is a falsifiable claim you can hand to the team that owns checkout, and it took four processes and no infrastructure.</p>
<p><b>Why each stage is the way it is.</b> <code>$1 &gt;= "2026-08-11T14:00:00Z"</code> is a <i>string</i> comparison. It is correct because ISO-8601 was designed so lexical order matches chronological order. <code>$7+0</code> coerces <code>"87ms"</code> to 87. <code>sort -k1,1 -k2,2n</code> sorts by route, then by duration numerically, so each route's durations arrive in ascending order and the final awk can index into them. The nearest-rank index is <code>int((n*99+99)/100)</code>, which is <code>ceil(0.99n)</code> in integer arithmetic. The whole thing streams: only the durations for the distinct routes are ever in memory.</p>
<p><b>What to do with it now.</b> Two habits to keep from this stream. <b>Build pipelines incrementally</b>: run stage one, look, add stage two, look again. A pipeline written in one go and debugged as a unit gives you a confident wrong answer. <b>When the same question recurs, stop reaching for the pipeline.</b> A one-off is what the command line is for. The third time you compute p99 by hand is the day to ask for latency histograms and per-route percentiles in the metrics stack.</p>`,
