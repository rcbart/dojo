...
behavior:`1. A parse error, and it aborts the whole run — jq is strict by design, so one stray startup banner or stack trace in the file kills a pipeline that worked yesterday. 2. grep '^{' is the pragmatic guard: cheap, streaming, and it keeps the malformed lines out. (jq's own -R with fromjson? can skip them instead, at more typing.) 3. Without //, a missing user key gives null, and null then flows into whatever you do next — string concatenation, comparisons, a group_by bucket named null. 4. null, on every line, silently. No error, no warning: your dashboard shows zero and your alert never fires. Log field names are an interface, and renaming one is a breaking change that no compiler will catch — which is why they belong in code review and in a schema, and why the JS Dojo lesson insists on a fixed set of fields. 5. --arg (and --argjson for non-string values). 6. Slurp. select streams one object at a time; -s materialises everything, so filter first and slurp only the survivors.`,
