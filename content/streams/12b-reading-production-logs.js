...
hints:['Question 1 is the reason the answer to "why does jq work with sed?" is "neither knows the other exists".','Question 2: if they ran one after the other, "grep | head" on a 40GB file could not possibly return instantly.','Question 6 is the honest limitation — the reason the last lesson of this stream switches to JSON.']}]},
...
sed -E 's#/api/orders/[0-9]+#/api/orders/:id#' app.log  # normalise ids, then count</div>
<p>That middle line is the idiom worth memorising: <b><code>-n</code> plus <code>s///p</code> is extraction</b> — match the line, keep only the captured group, print that. The third is the one that makes traffic countable: collapse <code>/api/orders/1234</code> and <code>/api/orders/9876</code> into a single normalised route, and suddenly <code>sort | uniq -c</code> tells you something true.</p>
<p><b>Where sed stops being the right tool.</b> Three places, and recognising them is most of the skill. <b>When you need fields</b> — "the sixth column" is awk's job; expressing it in sed means counting spaces in a regex. <b>When you need arithmetic or state across lines</b> — sums, averages, grouping. sed has no numbers. <b>When the input is structured</b> — HTML, JSON, CSV with quoted commas. A regex cannot parse nested structure, and the log line that breaks your pattern will be the one you needed.</p>
...
prompt:`Now change things. One command per numbered line: (1) replace every <code>trace=</code> id (pattern <code>trace=[0-9a-f]+</code>) with <code>trace=REDACTED</code>, using <code>sed -E</code>, (2) print <b>only</b> the duration digits from each line — combine <code>-n</code> with <code>-E 's/.* ([0-9]+)ms .*/\\1/p'</code>, (3) normalise numeric order ids: replace <code>/api/orders/1234</code>-style paths (regex <code>/api/orders/[0-9]+</code>) with <code>/api/orders/:id</code>, using <code>#</code> as the delimiter so you need no backslashes, (4) the <code>s///</code> flag that replaces <b>every</b> occurrence on a line rather than the first (one letter), (5) the flag that makes sed edit the file in place, with the warning that it has no undo (write the flag), (6) name the quantifier behaviour that makes <code>s/.*=//</code> delete through the <b>last</b> <code>=</code> on the line instead of the first (one word).`,
starter:`1.
2.
3.
4.
...
{title:'Normalise, then count',lang:'shell',diff:'medium',
prompt:`Ids make traffic uncountable; normalising them is sed's best day job. Assume lines like <code>... GET /api/orders/1234 200 87ms ...</code>. One command per numbered line: (1) rewrite any <code>/api/&lt;word&gt;/&lt;digits&gt;</code> path to <code>/api/&lt;word&gt;/:id</code> — use <code>sed -E</code> with <code>#</code> delimiters and the pattern <code>/api/([a-z]+)/[0-9]+</code>, replacement <code>/api/\\1/:id</code>, (2) pipe that into <code>awk '{print $5}'</code> to keep only the normalised route, (3) complete the frequency count by adding <code>sort | uniq -c | sort -rn</code>, (4) extract just the date part (the first 10 characters) and the level from each line, printing <code>date level</code>, using <code>sed -nE 's/^([^ ]{10}).*(ERROR|WARN).*/\\1 \\2/p'</code>, (5) strip the <code>ms</code> unit from durations so they can be sorted numerically — <code>sed -E 's/([0-9]+)ms/\\1/'</code>, (6) name what would happen to your route counts if you skipped step 1 entirely (one word: every request becomes its own ____ ).`,
starter:`1.
2.
3.
4.
...
<p><b>BEGIN and END</b> are patterns that match before the first line and after the last — where you print headers and totals. Variables need no declaration and start at <code>0</code> or <code>""</code>, so <code>{s+=$7; n++} END {print s/n}</code> is a complete average. And awk coerces strings to numbers by reading the leading digits: <code>"87ms"+0</code> is <b>87</b>. That single behaviour is why durations with units are not a problem here and are a problem everywhere else.</p>
...
behavior:`1. A parse error, and it aborts the whole run — jq is strict by design, so one stray startup banner or stack trace in the file kills a pipeline that worked yesterday. 2. grep '^{' is the pragmatic guard: cheap, streaming, and it keeps the malformed lines out. (jq's own -R with fromjson? can skip them instead, at more typing.) 3. Without //, a missing user key gives null, and null then flows into whatever you do next — string concatenation, comparisons, a group_by bucket named null. 4. null, on every line, silently. No error, no warning: your dashboard shows zero and your alert never fires. Log field names are an interface, and renaming one is a breaking change that no compiler will catch — which is why they belong in code review and in a schema, and why the JSDojo lesson insists on a fixed set of fields. 5. --arg (and --argjson for non-string values). 6. Slurp. select streams one object at a time; -s materialises everything, so filter first and slurp only the survivors.`,
