...
<p>When you launch a program, the operating system creates a <b>process</b>: an isolated container with its own private memory, its own file handles, and at least one <b>thread</b> of execution. A <b>thread</b> is a single sequential path through the code. It runs your instructions one after another.</p>
<p>The difference that matters is <b>memory</b>. Processes are <i>isolated</i>. One process can't read another&#8217;s memory, which makes them safe but heavyweight, and talking between them needs deliberate inter-process communication. Threads <i>within the same process</i> <b>share the process&#8217;s heap</b> (all its objects). Each thread gets its <b>own stack</b> for local variables and method calls. That shared heap makes threads useful and dangerous at once.</p>
<div class="codeSample">Process (isolated memory)
 ├─ Thread 1  → own stack ┐
 ├─ Thread 2  → own stack ├─ all share the SAME heap (objects)
 └─ Thread 3  → own stack ┘</div>
<p><b>Why use threads?</b> To use more than one CPU core at a time: a 4-core machine can run 4 threads at once, real parallelism. And to stay responsive: do slow I/O on one thread while another keeps the UI alive. The operating system switches threads on and off cores rapidly (a <b>context switch</b>), so even a single core can interleave many threads.</p>
<p>The catch is the shared heap. When two threads read and write the <i>same</i> object at the same time, you get <b>race conditions</b>: results that depend on unpredictable timing. Processes rarely have this problem because their memory is separate. Threads have it all the time, so synchronization is the heart of concurrency. In Java, the JVM itself runs as one process. Your program begins on the <code>main</code> thread, and you create more threads from there.</p>`,
...
<p><code>join()</code> blocks until the thread dies. It's the simplest coordination tool. Daemon threads (<code>setDaemon(true)</code> before start) don't keep the JVM alive. Interruption is cooperative. <code>t.interrupt()</code> sets a flag, and blocking calls throw <code>InterruptedException</code>, which you must handle: restore the flag or exit.</p>
<h4>What a thread costs</h4>
<p>A platform thread is a thin wrapper over an <b>operating system</b> thread. Creating one is a system
call. Each carries a stack reserved in megabytes, not kilobytes. Switching between them means the
kernel saving and restoring register state. That is why "just make a thread per request" was bad advice
for twenty years, and why thread <i>pools</i> exist: threads are expensive objects you want to reuse.</p>
<p>A few thousand platform threads is a lot. That number explains pool sizing, why blocking a thread felt wasteful enough to justify all of reactive programming, and why virtual threads (later in this stream) changed the advice rather than the language.</p>

<h4><code>start()</code> versus <code>run()</code></h4>
...
<p>After <code>start()</code> returns, you know nothing about what the new thread has done. Every coordination tool in this stream exists to replace that assumption with a guarantee.</p>

<h4>Interruption is a request</h4>
<p>There is no safe way to stop a thread from outside. <code>Thread.stop()</code> existed, could leave
objects half-modified with locks released, and was deprecated for that reason. So Java uses
...
<p><code>Thread</code> is the substrate, not the tool to reach for. Application
...
hand-rolled coordination. This lesson is the layer underneath, so those abstractions aren't mysterious.</p>`,
...
<p>No instruction reads, increments and writes a memory location atomically, so two threads can interleave and one increment disappears:</p>
...
<p>That interleaving is the default behavior, not a corner case. The window is nanoseconds wide, so it stays invisible in testing and appears under production load, on a different machine, where it can't be reproduced on demand. That is what makes concurrency bugs expensive.</p>

<h4>The second problem: visibility</h4>
<p>Atomicity is only half of it. Even a write that completes may never be <i>seen</i> by another thread.
The JVM and the CPU may keep values in registers and caches and reorder
...
ever changes it.</p>
<p>The Java Memory Model governs this. Its central concept is <b>happens-before</b>: unless
...
<h4>The rules</h4>
<p><b>Prefer not sharing.</b> Immutable objects
...
inside a <code>synchronized</code> block can acquire another lock and deadlock you. You won't find
...
order. A fixed acquisition order across the codebase makes it impossible. If you can't
...
blocked. On Java 21+ that is the case virtual threads solve, so you stop tuning pool
sizes for blocking work altogether.</p>

<h4>Exceptions vanish unless you look</h4>
<p>This surprises people most often. A task submitted with
...
<code>shutdownNow()</code> also interrupts them and returns the queued tasks that never ran.
Neither <i>waits</i>. That is <code>awaitTermination()</code>. The usual correct sequence is
shutdown, await a bounded time, then <code>shutdownNow()</code> if it hasn't drained.</p>
<p>"My main never exits" because pool threads are <b>non-daemon</b> by default, so the JVM
keeps running for them. Try-with-resources on Java 19+ handles this and is the simplest fix.</p>

<h4>The pool deadlock</h4>
...
<p>A thread waiting on a network call does nothing while holding a megabytes-sized stack and a slot in
your pool. Fan out to three services sequentially and you pay three latencies end to end when the
work could have overlapped. <code>CompletableFuture</code> lets you describe the <i>dependency graph</i>
between pieces of work and let the runtime schedule it, instead of hand-writing the coordination with
latches and futures you have to poll.</p>

<h4><code>thenApply</code> vs <code>thenCompose</code></h4>
...
<p>The <code>...Async</code> suffix matters too. <code>thenApply</code> may run the
callback on whichever thread completed the previous stage, in the worst case the caller's
thread. <code>thenApplyAsync</code> forces it onto an executor. If a callback does anything slow, use the
async variant <b>and pass your own executor</b>. The default is the common ForkJoinPool, which is
sized for CPU-bound work and shared with parallel streams across the entire JVM.</p>

<h4>Failure</h4>
<p>An exception doesn't propagate. It <i>completes the future exceptionally</i>, and every downstream
stage is skipped until something handles it. So an unhandled failure is silent unless you
consume the result: no stack trace, no log line, a value that never arrives.</p>
...
<p>The two ways to wait differ. <code>get()</code> throws checked exceptions,
...
<h4>What it doesn't give you</h4>
<p>No cancellation that propagates to work already running. No timeout before Java 9's
<code>orTimeout</code>. No structural relationship between a task and its subtasks, so an orphaned
branch of the graph can keep running after you've stopped caring. Java 21's <b>structured concurrency</b>
and virtual threads address that. For straightforward fan-out on a virtual thread, ordinary
blocking calls are now simpler and easier to debug. Reach for
...
<p><code>ConcurrentHashMap</code> beats <code>Collections.synchronizedMap</code>: it's striped, and compound ops like merge/compute are atomic. <code>AtomicInteger/Long</code> for counters, <code>LongAdder</code> under heavy contention. <code>CountDownLatch</code> = one-shot "wait for N events"; <code>Semaphore</code> = permits; <code>CyclicBarrier</code> = reusable meeting point.</p>
...
<p><code>ConcurrentHashMap</code> beats <code>synchronizedMap</code> on speed. More
importantly, it <i>offers</i> the atomic compound operations (<code>computeIfAbsent</code>, <code>merge</code>,
...
CAS starts failing and retrying. That is why <b><code>LongAdder</code> outperforms
...
<p>A latch can't be reset. So "wait for startup" is a latch and "wait for
everyone each round" is a barrier. Using a latch where you needed a barrier produces code that works
once.</p>
<p><b>Prefer structure to primitives.</b> Most code that reaches for a latch wants
<code>invokeAll</code> on an executor, or <code>CompletableFuture.allOf</code>. Use the primitives when
the higher-level tools don't fit.</p>`,
...
<p><code>synchronized</code> is a lock you can't see. There is no object to name, no way to ask whether it's held, no way to give up waiting, and one waiting queue per monitor. That is fine for the common case and runs out of road quickly. The <code>java.util.concurrent.locks</code> package gives you the same mutual exclusion as an <i>object with methods</i>. Those extra methods are the reason to reach for it.</p>
...
<p>Reentrant means the same thread may acquire it again without deadlocking itself, as <code>synchronized</code> already allowed. The hold count has to fall back to zero before anyone else gets in.</p>

<h4>What a monitor can't give you</h4>
...
<p><code>tryLock</code> is the one that changes designs. Deadlock requires a thread to wait indefinitely for a lock somebody else holds. A timed <code>tryLock</code> with a back-off replaces "wait forever" with "fail, release what I already hold, and try again". That is how you survive a lock-ordering problem you can't fix by reordering, and why bank-transfer code in real systems so rarely uses plain <code>synchronized</code>.</p>

<h4>The rule, and the two ways people break it</h4>
<p>Acquire <i>outside</i> the <code>try</code>, release <i>inside</i> the <code>finally</code>. Put <code>lock()</code> inside the try and an exception thrown by the acquisition itself sends you to a <code>finally</code> that unlocks a lock you never took. <code>unlock</code> on a lock you don't hold throws <code>IllegalMonitorStateException</code>, which buries the original failure. The second mistake belongs to <code>tryLock</code>. The shape changes because you must not release what you failed to acquire:</p>
...
<h4>ReadWriteLock, and when it pays</h4>
<p>A <code>ReentrantReadWriteLock</code> admits many readers at once and writers alone. That sounds like a free win and usually isn't. The extra bookkeeping costs more than a plain lock, so it pays only when reads dominate <i>and</i> each read holds the lock long enough for the concurrency to matter. For a short read of two fields, a plain <code>ReentrantLock</code>, or an immutable snapshot swapped through an <code>AtomicReference</code>, will beat it. <code>StampedLock</code> goes further with an optimistic read that takes no lock at all. The price: it isn't reentrant, and you must write the validate-and-retry protocol correctly.</p>

<h4>Condition: waiting for a state</h4>
...
<p>Wait in a <code>while</code> loop that re-tests the condition. A thread may wake with nobody having signaled it, and another thread can take the slot between the signal and your reacquiring the lock. And signal the queue you mean: a consumer that just removed an item signals <code>notFull</code>, not <code>notEmpty</code>.</p>

<h4>Which to reach for</h4>
<p>Start with <code>synchronized</code>. It's shorter, it can't leak, and the JVM optimizes it aggressively. Move to a <code>ReentrantLock</code> when you need a timeout, a non-blocking attempt, interruptible waiting, fairness, or more than one condition. There is now a third reason, developed in the next lesson: blocking inside <code>synchronized</code> can pin a virtual thread to its carrier, while a <code>ReentrantLock</code> lets the JVM unmount cleanly. Before any of that, look one level up. A <code>BlockingQueue</code> is the bounded buffer sketched above, already written, already correct, already tested by everyone.</p>`,
...
<p>Rules of thumb: virtual threads for I/O-bound fan-out (API calls, DB queries). Fixed platform pools are still fine for CPU-bound work. Don't pool virtual threads; create one per task. Avoid long <code>synchronized</code> blocks around blocking calls (pinning; use ReentrantLock if needed). Spring Boot 3.2+: <code>spring.threads.virtual.enabled=true</code>.</p>
<h4>Why this changes the advice</h4>
...
stack traces that say nothing, and a context threaded by hand.</p>
<p>Virtual threads remove the trade. When one blocks, the JVM <b>unmounts</b> it, stack and all, and reuses the carrier for someone else. The
...
<p>A virtual thread can't always be unmounted. If it blocks inside a <code>synchronized</code> block or a
native frame, it is <b>pinned</b> to its carrier. If enough of them pin at once, you've starved the
...
<h4>What virtual threads don't fix</h4>
<p><b>They aren't faster for CPU-bound work.</b> You still have the cores you have. A fixed platform pool
sized to them remains correct.</p>
<p><b>They remove your accidental rate limit.</b> A 200-thread pool was also a cap of 200
concurrent calls to the downstream service. Replace it with unbounded virtual threads and you'll
discover what that cap was protecting: the database connection pool is now the bottleneck, or you're
...
threads, per-thread copies are memory you didn't budget for. <b>Scoped values</b> are the modern
replacement for passing context.</p>
<p><b>Structured concurrency</b> completes the picture. Subtasks forked in a scope are guaranteed to
finish or be canceled before the scope exits, so a failed fan-out can't leave orphaned work running.</p>`,
...
<p>Virtual threads made blocking cheap, but they did nothing about <b>shape</b>. Fan out to three services with an executor and you've created three tasks that outlive nothing in particular. If the first fails, the other two keep running, spending money and holding connections. The only thing that eventually stops them is that nobody is listening. An ordinary method call is different: a thrown exception unwinds everything below it automatically.</p>
<p><b>Structured concurrency</b> restores that property. Subtasks are forked inside a <b>scope</b>, and the scope doesn't exit until every subtask has finished or been cancelled. Concurrency gets a block structure, the way loops and try-blocks gave control flow one.</p>
...
<p>Each consequence of that invariant fixes a real complaint about <code>CompletableFuture</code>. <b>No leaks</b>: an orphaned branch can't exist, because the scope won't close while one is running. <b>Cancellation propagates</b>: down to the subtasks and, on failure, sideways to the siblings. <b>The relationship is visible</b>: a thread dump groups the subtasks under their parent instead of listing forty anonymous pool threads.</p>

<h4>The API, and a caveat</h4>
<p><code>StructuredTaskScope</code> lives in <code>java.util.concurrent</code> and has been a <b>preview API</b> across several releases. You enable it with <code>--enable-preview</code>, and its surface has changed between JDKs. The JDK 21 form used <code>ShutdownOnFailure</code> and <code>ShutdownOnSuccess</code> subclasses. Later releases replaced those with a factory taking a joiner policy. Learn the model rather than the method names, check the API against the JDK you run, and expect one migration when it finalizes.</p>
...
<p><code>ThreadLocal</code> was the standard way to carry a request id or a user principal down a call stack without threading a parameter through twenty methods. With a million virtual threads it stops being tolerable. Every one carries its own mutable map, nothing forces the value to be cleared, and an inherited thread-local copies the whole map to each child.</p>
...
<p><b>Make it reproducible.</b> A bug you can't trigger you can't fix. Amplify the race: run the operation in a tight loop across many threads, add small random sleeps to widen the timing window, and assert an invariant. For example, launch N threads that each increment a counter and check the total equals N. If the total comes out low, you've proven a lost update.</p>
<p><b>Reach for the right tool:</b></p>
<ul>
<li><b>jstack</b> (or a thread dump): snapshots every thread's stack. The JVM even prints "Found one Java-level deadlock" and names the two threads and locks. That makes deadlocks the <i>easy</i> case once you capture the dump.</li>
<li><b>jcstress</b>: the OpenJDK harness built to stress-test concurrent code against the Java Memory Model and surface races real tests miss.</li>
<li><b>AtomicInteger / atomics</b>: swapping a plain counter for an atomic fixes lost updates and, in a test, confirms the plain version was racy.</li>
