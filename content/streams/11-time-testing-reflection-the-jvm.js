...
<p><b>Serialization</b> turns an object graph into bytes. <b>Deserialization</b> turns bytes back into objects. Java ships a native mechanism:</p>
...
<p><code>Serializable</code> is a marker interface with no methods. <code>serialVersionUID</code> pins compatibility. Omit it and the JVM computes one from the class structure, so adding a field or a method changes it and old data fails with <code>InvalidClassException</code>. <code>transient</code> excludes secrets and caches. The whole reachable object graph gets serialized, stray references included.</p>
<p>Deserializing untrusted bytes is remote code execution waiting to happen. Never <code>readObject</code> external input. Modern systems serialize through explicit formats instead: JSON via Jackson (your api3 lesson), or Protobuf/Avro for compact schema-versioned data. Records + Jackson is the default. Native serialization survives mainly in caches, session replication, and legacy RPC.</p>

<h4>Why the industry moved away from native serialization</h4>
<p><b>Security</b>: <code>readObject</code> constructs arbitrary types before your code sees anything. An attacker who controls the bytes controls what gets built. That is the gadget chain problem. The JDK added <code>ObjectInputFilter</code> in response, for the cases where you must deserialize. <b>Coupling</b>: the format is your class structure, so a rename breaks data at rest. <b>Interoperability</b>: nothing outside the JVM can read it.</p>
<p>Explicit formats invert all three. JSON, Protobuf and Avro have a schema you version deliberately, a parser that builds only what you asked for, and readers in every language.</p>

<h4>Schema evolution is the real subject</h4>
<p>Serialization is easy once. The engineering is version two, while old and new code run at the same time. The rules that hold across formats:</p>
<ul>
<li><b>Adding an optional field is safe.</b> Old readers ignore it. New readers default it.</li>
<li><b>Removing or renaming a field is not.</b> Deprecate, stop writing it, wait for consumers, then remove: the same expand-and-contract shape as a database migration.</li>
<li><b>Never reuse a field number or a name</b> for a different meaning. Protobuf makes this explicit with <code>reserved</code>. JSON leaves you to remember.</li>
<li><b>Tolerant readers win.</b> A consumer that ignores unknown fields lets producers move first, so teams can deploy independently.</li>
</ul>

<h4>Choosing a format</h4>
<p><b>JSON</b> is the default for anything crossing an organizational boundary: readable, debuggable, supported everywhere. The cost is size and no schema unless you add one. <b>Protobuf or Avro</b> earn their keep on high-volume internal traffic and event streams, where a schema registry gives you compatibility checks in CI rather than incidents in production. Decide by whether you need a contract a machine can enforce. Performance rarely settles it.</p>`,
...
<p>Rule one: store <code>Instant</code> (UTC) in databases and apply zones only at display. <b>UTC</b> is Coordinated Universal Time: the one clock everything is logged and compared in, so two servers in different time zones agree on "when".</p>
<h4>Why the old API had to be replaced</h4>
<p>The fixes explain the new design. <code>Date</code> was <b>mutable</b>, so a method you passed one to could change it under you, and it was shared across threads with no safety. <code>Calendar</code> numbered months from zero, so December was 11 and off-by-one errors were the norm. <code>SimpleDateFormat</code> was not thread-safe. It looked stateless, so people stored it in a static field, which produced corrupted dates under load and nowhere else.</p>
<p><code>java.time</code> answers all three. Everything is <b>immutable</b>, therefore thread-safe. Formatters are immutable and safe to share. The type you choose states what you mean.</p>
...
<p><b>Forgetting the return value.</b> Every method returns a new object. <code>d.plusDays(1);</code> as a statement does nothing and compiles cleanly. It is the most common java.time bug and the easiest to miss in review.</p>
<p><b>Period and Duration disagree, correctly.</b> Adding one month to 31 January gives the last day of February, 28 or 29 depending on the year. Adding 30 days gives 2 March, or 1 March in a leap year. Both are right. Only one is what you meant. Across a daylight-saving boundary, adding <code>Period.ofDays(1)</code> keeps the wall-clock time while <code>Duration.ofHours(24)</code> shifts it by an hour.</p>
<p><b>Storing local times.</b> A meeting stored as an <code>Instant</code> is wrong if the government moves the clocks after you scheduled it. The user meant "9am on the 3rd in Berlin", so store the local time and the zone id, and resolve to an instant at use. An event that already happened is an <code>Instant</code>, always.</p>
<p><b>Zone ids, not offsets.</b> <code>ZoneId.of("Europe/Berlin")</code> carries the DST rules. <code>+01:00</code> is a fixed offset that will be wrong for half the year. Those rules change by political decision, so keep the tzdata in your JDK and containers current.</p>

<h4>The one change that makes time testable</h4>
<p>Inject a <code>Clock</code> rather than calling <code>now()</code> directly. Every <code>now()</code> method accepts one. Production passes <code>Clock.systemUTC()</code> and tests pass <code>Clock.fixed(...)</code>. That removes a whole category of untestable code (expiry windows, rate limits, scheduling) for the price of one constructor parameter.</p>`,
...
<p><b>What makes a test worth having.</b> A good test fails for one reason, names that reason in its title, and does not change when you refactor the implementation. That last property is the one most often lost. A test that asserts on internal calls rather than observable behavior breaks every time you improve the code, and a suite that punishes refactoring gets deleted or ignored.</p>
...
<p>Mock things that are slow, non-deterministic, or outside your process: clocks, networks, payment providers, the filesystem. Mocking your own value objects and calculators produces tests that pass while the system is broken.</p>

<h4>Determinism: the two usual culprits</h4>
<p><b>Time</b> and <b>randomness</b> make tests flaky, and both are fixable by injection. Take a <code>Clock</code> rather than calling <code>Instant.now()</code>, and a <code>Random</code> with a fixed seed. <code>Clock.fixed(...)</code> in a test turns "expires tomorrow" from a guess into an assertion. It also lets you test the boundary, which is where date bugs live.</p>
...
assertAll(() -&gt; assertEquals(...), () -&gt; assertEquals(...));  // report ALL
                                                               // failures, not
                                                               // just the first</div>
<p><code>@ParameterizedTest</code> is the highest-value habit here. It turns five near-identical tests into one with five rows, and adding a case becomes a line rather than a method.</p>

<p>Structure every test as Arrange-Act-Assert. Name tests after behavior, not methods. Constructor injection, from the Spring stream, is what makes mocking possible.</p>`,
...
<p>Annotations are structured metadata. Reflection reads types at runtime. Together they power Spring, JUnit and Jackson.</p>
...
<p>Without <code>RUNTIME</code> retention, <code>getAnnotation</code> returns null. That is the most common custom-annotation bug.</p>
<h4>Why the two are always discussed together</h4>
<p>Separately they are unremarkable. An annotation is inert metadata, and reflection is a slow way to do what a normal method call does better. Together they are the mechanism behind every framework in this course: <b>you declare intent, and something else discovers it and supplies behavior</b>. JUnit finds your <code>@Test</code> methods this way. Spring finds beans and transactional methods. Jackson finds property names.</p>
<p>No framework has access to a mechanism you do not. What they have is the discipline to cache the reflection and a lot of edge-case handling.</p>

<h4>Retention decides whether any of it works</h4>
...
<p><code>@Target</code> is the other half. It makes misuse a <b>compile error</b> rather than an annotation that silently does nothing.</p>

<h4>The costs</h4>
<p>Reflection is slower than direct invocation. Lookups are expensive, and while modern JITs optimize repeated calls well, the discovery phase is not free. It also <b>defeats the compiler</b>: a field renamed by a refactoring tool leaves a string somewhere that no longer matches, and you find out at runtime. And it can breach encapsulation. The module system (later in this course) restricts deep reflection by default for that reason, and <code>setAccessible(true)</code> increasingly needs an explicit <code>--add-opens</code>.</p>
<p>So frameworks scan once at startup and cache the result. Application code should reach for an interface or a functional parameter first. If you are reflecting to call one of your own methods, there is usually a design that does not need to.</p>

<h4>The modern alternatives</h4>
<p><b>Annotation processors</b> read the same annotations at compile time and generate real code: fast, checked, debuggable, and the basis of MapStruct and Micronaut. <b>Method handles</b> and <code>VarHandle</code> give reflective flexibility at close to direct-call speed. Know both exist before you write a reflection-heavy library.</p>`,
...
<ul>
<li><b>Meta-annotations</b>. <code>@Repeatable</code> lets you apply the same annotation twice and needs a container annotation. <code>@Inherited</code> lets subclasses see the superclass's class-level annotation. <code>@Documented</code> shows it in javadoc.</li>
<li><b>Composed annotations</b>: annotate an annotation to bundle behavior. This is how Spring works, and how you build your own <code>@AdminEndpoint</code> (below).</li>
<li><b>Elements</b> can have <code>default</code> values. A single element named <code>value</code> enables the shorthand <code>@Audited("x")</code>.</li>
...
<p>The machinery above lets you express a policy <b>once</b> and apply it declaratively. Instead of six lines of audit code repeated in forty methods, one of which someone will forget, there is one annotation and one implementation. The omission is visible because the annotation is missing.</p>
<p>The composed-annotation pattern is how the frameworks are built, all the way down:</p>
...
<p>Spring resolves meta-annotations recursively, so this needs no code. Plain Java does <b>not</b>. <code>getAnnotation()</code> looks only at what is directly present, so outside a framework you must walk the annotations on the annotation yourself.</p>

<h4><code>@Inherited</code>, and why it disappoints</h4>
<p>It sounds like "subclasses see this" and is much narrower. It applies only to <b>class-level</b> annotations, only for <code>getAnnotation</code> (not <code>getDeclaredAnnotation</code>), and <b>never</b> to methods or interfaces. An annotation on an interface is never inherited by implementing classes. Frameworks search the hierarchy themselves. Spring's <code>AnnotatedElementUtils</code> exists for this and finds things plain reflection will not.</p>

<h4>What annotations cannot do</h4>
<p>Element types are restricted: primitives, <code>String</code>, <code>Class</code>, enums, other annotations, and arrays of those. No arbitrary objects, and <b>no nulls</b>, so an "absent" value has to be signaled by a default like <code>""</code>. Values must be compile-time constants. That is why you see <code>Class</code> literals and string expressions (like Spring's SpEL) rather than lambdas.</p>

<h4>When not to reach for one</h4>
<p>Annotations move behavior away from the code it affects. That is their value and their cost. A reader looking at the method cannot see what else happens to it, and a typo in an annotation-driven behavior fails silently. Use them for cross-cutting, orthogonal concerns (auditing, transactions, security, serialization). Do not use them to configure business logic, where an ordinary parameter is clearer and the compiler is on your side.</p>`,
...
<p>What happens to your code:</p>
<ul>
<li><b>Compilation</b>: javac → bytecode (.class) → JVM interprets, then the <b>JIT</b> compiles hot paths to native code. The JIT is the just-in-time compiler: the JVM starts by interpreting bytecode, watches which methods run hot, and compiles those to native machine code while the program runs. That is why Java "warms up", and why the first seconds of a benchmark lie.</li>
<li><b>Memory</b>: objects live on the <b>heap</b>. Each thread has a <b>stack</b> of frames. Both are explained below.</li>
...
<h4>Why "compiled and interpreted" matters</h4>
<p>The hybrid is what makes Java fast in long-running processes. Bytecode starts interpreted, which is slow but needs no analysis. Meanwhile the JVM <b>profiles</b> which branches are taken and which types appear at each call site. Once a method is hot, the JIT compiles it to native code using that evidence.</p>
<p>The result is optimization a static compiler cannot perform. An interface call that has only ever seen one implementation is compiled as a direct call and inlined. Branches never taken are compiled away. If the assumption later proves wrong, the JVM <b>deoptimizes</b> and recompiles. This is why Java often beats naively-written C++ in long-running services, and why the first thousand requests are slower than the rest: the "warm-up".</p>
...
<h4>Memory: the two regions</h4>
<p><b>Heap</b> holds objects and is shared by every thread. The garbage collector manages it and <code>-Xmx</code> sizes it. <b>Stack</b> is per-thread, holds frames with locals and references, and is freed as calls return. So <code>StackOverflowError</code> is runaway recursion, and <code>OutOfMemoryError: Java heap space</code> is either a capacity problem or a leak.</p>
<p>"Leak" in Java means <b>an unintended reference</b>, not forgotten frees. The usual culprits: a static collection that only grows, a cache with no eviction, a listener never unregistered, a <code>ThreadLocal</code> not cleared on a pooled thread. The tool is a heap dump. Take it with <code>-XX:+HeapDumpOnOutOfMemoryError</code> and open it in Eclipse MAT, which names the retaining path.</p>
<p>The heap is not all of it. Metaspace, thread stacks, code cache and direct byte buffers live <b>outside</b> it. A container with <code>-Xmx</code> set to the memory limit therefore gets OOM-killed by the kernel with nothing in the application log.</p>

<h4>GC, and the only tuning advice that survives contact</h4>
<p><b>GC</b> is garbage collection: the JVM finds objects nothing can reach any more and frees their memory for you. The price is pauses, which is why heap size and collector choice matter under load. Generational collection exploits one empirical fact: <b>most objects die young</b>. New objects go in eden, survivors are copied a few times, and the persistent minority are promoted to the old generation. The common case is collecting a small region where almost everything is garbage, which is cheap.</p>
<p>Do not call <code>System.gc()</code>. Do not copy JVM flags from a blog. Choose a collector by requirement: <b>G1</b> as the default, <b>ZGC</b> when pause time matters more than throughput, <b>Parallel</b> for batch throughput. Then <b>measure before changing anything else</b>. Most "GC problems" are allocation problems, and the fix is in your code.</p>

<h4>The tools to reach for at 3am</h4>
<p><code>jcmd &lt;pid&gt; help</code> lists everything a live JVM will tell you. <code>jstack</code> gives a thread dump when things are hung. Take three, thirty seconds apart, and look for threads stuck in the same place. <b>JFR</b> (<code>jcmd &lt;pid&gt; JFR.start</code>) is the one to learn properly: continuous, low-overhead profiling you can leave running in production and inspect afterwards in JDK Mission Control.</p>`,
