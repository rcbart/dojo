...
<p><b>Git is a version control system</b>: a program that records snapshots of your project. You can see every change ever made, undo any of them, and work on several ideas in parallel without copies like <code>Main_final_v2_REALLY.java</code>. Linus Torvalds built it in 2005 to manage Linux. Today it's effectively the only game in town.</p>
<p><b>Why you need it</b>, even alone, even for one file: a complete history of who changed what and why, and fearless experimentation, since any state is recoverable. Once remotes enter, it's also the collaboration backbone of the industry. GitHub, GitLab, CI/CD, code review: all of it sits on git.</p>
<p><b>The model, in four ideas:</b></p>
<ul>
<li><b>Snapshots, not diffs.</b> A <b>commit</b> is a full snapshot of every tracked file, plus a message, an author, a timestamp, and a pointer to its parent commit. Diffs are <i>computed</i> between snapshots, not stored. Each commit is named by a hash like <code>a3f9c21</code>. Those are the ids you'll pass to commands.</li>
<li><b>Three areas.</b> Your <b>working directory</b> is the files you edit. The <b>staging area</b> is what you've marked for the next commit with <code>git add</code>. The <b>repository</b> is the committed history, stored in the hidden <code>.git/</code> folder. The two-step add-then-commit lets you compose what each commit contains.</li>
<li><b>Branches are pointers.</b> A branch is a movable label on a commit. <b>HEAD</b> is "where you are now". Creating a branch copies nothing: it writes a 41-byte file. That's why git branching is fast and habitual.</li>
<li><b>Everything is local.</b> Your clone contains the FULL history. Commits, diffs, branches, log: all instant, all offline. Remotes, which arrive two lessons from now, are other copies you exchange commits with.</li>
</ul>
...
<p>The core loop of a git life, with a single file. Create a project and watch one file travel <i>untracked → staged → committed → modified → staged → committed</i>. Run every command here for real if you have a terminal nearby. The drill below asks you to reproduce them.</p>
...
<p>Checking <code>git diff --staged</code> before every commit is the habit that prevents "oops, that wasn't meant to go in".</p>
...
<p>A <b>snapshot</b> of the whole tracked tree, plus a pointer to its parent, an author, a
timestamp and a message. Diffs are computed on demand by comparing snapshots. That's why branching is
...
<h4>Messages</h4>
<p>Write the subject in the imperative ("Add notes", not "Added some stuff"). Git's own generated
messages read that way, and the line completes the sentence "this commit will…". Then, when it isn't
obvious, say <b>why</b> in the body. The what is in the diff and always will be. The why exists only in
your head until you write it down. It's what the next person needs, including you in six months
reading <code>git blame</code> on a line you don't remember writing.</p>`,
...
<p>Real projects have many files. The staging area exists so that <b>a commit contains one logical change, not "everything I touched today"</b>. Add a second file to the journal and learn to compose commits deliberately.</p>
...
<p>The habits this makes possible:</p>
<ul>
<li><b>Path-scoped staging</b>: <code>git add &lt;file&gt;</code> per file. Use <code>git add .</code> only when you've checked status first and mean "all of it".</li>
...
<li><b>Review shape</b>: <code>git log --stat</code> shows which files each commit touched. A commit touching 14 files named "fix typo" is lying about something.</li>
</ul>

<h4>Why the staging area exists</h4>
<p>Most version-control systems commit everything you changed. Git inserts a step between
"changed" and "recorded" so a commit can be <i>composed</i> rather than captured. The commit is the unit of review, revert and cherry-pick. One logical change
can be understood in isolation and undone without collateral damage. Everything you did
on Tuesday cannot.</p>
...
<p><code>git add -p</code> makes this practical. Work rarely arrives in tidy units. You
fix a bug, notice a typo, rename a variable. Hunk-level staging lets you separate them
afterwards.</p>

<h4>The habit that prevents the embarrassing commit</h4>
<p>Read <code>git diff --staged</code> before every commit. It's the last moment before the change becomes
history, and where you catch the debug print, the commented-out block, the credential you pasted in
while testing. Two minutes there saves a follow-up commit titled "remove debugging", or a rotation of a
leaked key.</p>
<p><code>git add</code> records the content <i>at that moment</i>. Edit the file
afterwards and the later edit isn't staged. That's why <code>git status</code> can show one file as both
staged and modified.</p>`,
...
<p>Everything so far lived in <code>.git/</code> on your machine. A <b>remote</b> is <i>another copy of the repository</i>, usually on a server (GitHub, GitLab), that you exchange commits with. Your local repo is complete and independent. The remote is a peer you sync with, not a "master copy" you check files out from.</p>
<ul>
<li><b>origin</b>: the default nickname for the remote you cloned from. <code>git remote -v</code> lists remotes and their URLs. It's only a name, and you can add more. Fork workflows add an <code>upstream</code> remote.</li>
<li><b>clone</b>: copy a remote repo, full history included: <code>git clone git@github.com:acme/app.git</code>.</li>
<li><b>push</b>: send your new local commits to the remote. The first push of a new branch sets its <b>upstream</b>, the remote branch it tracks: <code>git push -u origin feature/todo</code>. After that, plain <code>git push</code> and <code>git pull</code> know where to go.</li>
...
<p>A branch lets an idea develop in isolation while <code>main</code> stays shippable. Branches are pointers, so the cycle is cheap enough for every feature, fix and experiment. It's the industry's default workflow:</p>
...
<p><b>Two kinds of merge.</b> If main hasn't moved since you branched, git slides the pointer forward: a <b>fast-forward</b>, no new commit. If main HAS moved, git creates a <b>merge commit</b> with two parents, joining the histories. Both are normal. <code>git log --graph --oneline</code> shows the shape.</p>
<p><b>Conflicts.</b> If both branches changed the <i>same lines</i>, git stops and marks the file:</p>
<div class="codeSample">&lt;&lt;&lt;&lt;&lt;&lt;&lt; HEAD
day 2: shipped the feature
=======
day 2: fixed the bug
&gt;&gt;&gt;&gt;&gt;&gt;&gt; feature/tags</div>
<p>Nothing is broken. Edit the file to the text you want, remove the markers, <code>git add</code> it, <code>git commit</code>. <code>git merge --abort</code> backs out entirely if you want to think first.</p>
<p>Branch hygiene: short-lived branches, named by intent (<code>feature/…</code>, <code>fix/…</code>), merged within days.</p>

<h4>What a merge conflict is</h4>
...
same region. No answer can be derived from the history, so git declines to guess.</p>
<p>So the resolution is rarely "pick a side". Usually you write the version that expresses
both intentions, then run the tests. A syntactically clean merge of two logically incompatible
changes compiles and behaves wrongly.</p>

<h4>Merge or rebase</h4>
<p><b>Merge</b> preserves what happened, including the fact that two lines of work existed. It
creates a merge commit and the history is a graph. <b>Rebase</b> replays your commits onto the new base,
producing a straight line that is easier to read and that never happened. Both are defensible. Rebase rewrites commits, though, so it belongs to work you haven't shared.</p>

<h4>Keeping branches cheap</h4>
<p>The cost of a branch is the <b>divergence</b>, and divergence grows with time.
A branch open for a day merges itself. One open for three weeks becomes a negotiation. Merging main into your branch daily, or rebasing onto it, converts
one large painful merge into a series of trivial ones. That's the argument for continuous integration.</p>
<p>Delete branches after merging. Git keeps the commits regardless, and two hundred stale branches make it impossible to tell
which represent work in progress.</p>`,
...
<p>Git has several undo tools because "undo" means different things. Matching the tool to the situation is the skill.</p>
<ul>
<li><b>Discard uncommitted edits to a file</b>: <code>git restore notes.txt</code>. The working copy resets to the last commit. The one command here that destroys work, since those edits were never committed. Give it a breath before Enter.</li>
<li><b>Unstage, keep the edits</b>: <code>git restore --staged notes.txt</code>. The file comes back out of the staging area. Your changes stay in the working directory.</li>
<li><b>Undo a PUSHED commit</b>: <code>git revert abc123</code>. A NEW commit applying the inverse. History moves forward and teammates' clones stay consistent. The only polite undo for shared history. The full story is in the advanced lesson.</li>
...
<p><b>Has it been pushed?</b> Unpushed history is yours to
...
<code>revert</code>. Force-pushing a shared branch is a problem you have handed to every
colleague at once.</p>
...
<p>Only the last one destroys anything, and only what was never committed. <b>A commit is almost impossible to lose.</b> Even after a bad reset or rebase, the commit
still exists, <code>git reflog</code> knows where HEAD has been, and recovery is usually
<code>git reset --hard abc123</code> away.</p>

<h4>What cannot be recovered</h4>
<p>Uncommitted working-tree changes wiped by
<code>reset --hard</code> or <code>restore</code>. Untracked files removed by <code>git clean -fd</code>.
Anything from before your first commit. Everything else is reachable. So commit
early and often. Even a rough commit turns a destructive mistake into a recoverable one, and you can tidy
the history afterwards while it's still unpushed.</p>`,
...
<p>The advanced toolbox splits in two: commands that <b>move history forward</b>, safe anywhere, and commands that <b>rewrite history</b>, sharp-edged and local-only. The golden rule that keeps teams sane: <b>never rewrite commits that others may already have</b>.</p>
<ul>
...
<li><b>git reset</b>: moves your branch pointer backwards, in three strengths. <code>--soft HEAD~1</code> uncommits and keeps changes staged, right for "forgot a file" or "wrong message". <code>--mixed</code>, the default, uncommits and unstages, changes kept in files. <code>--hard</code> throws it all away, and reflog is your only friend after this. Local branches only.</li>
...
<p>Half of senior Git is <b>reading</b> history: finding when a line changed, why, and by whom. The workhorse is <code>git log</code>. It has far more range than the default wall of text.</p>
<ul>
...
<p>The everyday investigation flow. <code>git log -S brokenFn</code> to find the change. <code>git show</code> that hash to read it, or <code>git blame</code> the file to pin the line. <code>git reflog</code> when you need to undo an undo.</p>`,
