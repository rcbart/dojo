...
<p>Not a diff but a <b>snapshot</b> of the whole tracked tree, plus a pointer to its parent, an author, a
...
<ul>
<li><b>Discard uncommitted edits to a file</b>: <code>git restore notes.txt</code>. The working copy resets to the last commit. The one command here that genuinely destroys work (those edits were never committed), so give it a breath before Enter.</li>
<li><b>Unstage, keep the edits</b>: <code>git restore --staged notes.txt</code>. This pulls the file back out of the staging area; your changes stay in the working directory.</li>
<li><b>Undo a PUSHED commit</b>: <code>git revert abc123</code>. This creates a NEW commit applying the inverse. History moves forward, teammates' clones stay consistent. The only polite undo for shared history (the full story in the advanced lesson).</li>
