...
tests:[{d:'the table has an extra row and column',re:'length\\s*\\+\\s*1'},{d:'characters are compared with the minus-one offset',re:'a\\[i\\s*-\\s*1\\]|charAt\\s*\\(\\s*i\\s*-\\s*1'},{d:'a match extends the diagonal',re:'dp\\[i\\s*-\\s*1\\]\\[j\\s*-\\s*1\\]\\s*\\+\\s*1'},{d:'a mismatch takes the better neighbor',re:'Math\\.max\\s*\\(\\s*dp\\[i\\s*-\\s*1\\]\\[j\\]'}],
