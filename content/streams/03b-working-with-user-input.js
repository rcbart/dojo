...
<p>Programs earn their keep by reacting to input. On the console, input arrives through <code>System.in</code>, a raw byte stream. <code>Scanner</code> is the standard tool that turns those bytes into values you can use.</p>
<p>Scanner reads in two different modes. Mixing them is where everyone gets burned.</p>
<ul>
...
<p>The second habit: <b>ask before you read</b>. Every <code>nextInt()</code> on non-numeric input throws <code>InputMismatchException</code>, and the bad token <i>stays in the stream</i>. A naive retry loops forever on the same garbage. The professional pattern pairs <code>hasNextInt()</code> with an explicit discard:</p>
...
<p>A design habit from the Fundamentals prompt exercise: methods that read input should <b>take the Scanner as a parameter</b>. One Scanner per program, passed around, never closed, because closing it closes System.in for everyone. It's also testable: hand in <code>new Scanner("36\\nAda\\n")</code>.</p>`,
...
tests:[{d:'readInt guards with hasNextInt before reading',re:'while\\s*\\(\\s*!\\s*sc\\.hasNextInt\\s*\\(\\s*\\)\\s*\\)'},{d:'Bad tokens discarded with sc.next() inside the loop',re:'readInt\\s*\\([^)]*\\)\\s*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?while(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?sc\\.next\\s*\\(\\s*\\)\\s*;'},{d:'Returns via nextInt only after the guard',re:'return\\s+sc\\.nextInt\\s*\\(\\s*\\)'},{d:'readName clears the dangling newline first (two nextLine calls)',re:'sc\\.nextLine\\s*\\(\\s*\\)\\s*;(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?return\\s+sc\\.nextLine\\s*\\(\\s*\\)'},{d:'Scanner is a parameter, never constructed inside',re:'new\\s+Scanner\\s*\\(',not:true},{d:'Scanner never closed',re:'sc\\.close|\\.close\\s*\\(\\s*\\)',not:true}],
...
<p>The professional touches in that snippet. Complaints go to <b><code>System.err</code></b>, so piped stdout stays clean. A <b>usage line</b> tells the user what legal input looks like. <b><code>System.exit</code> with a non-zero code</b> lets shells and CI detect the failure: 0 means success, anything else means "went wrong", by universal convention.</p>
<p>The third input door is <b>environment variables</b>: <code>System.getenv("API_KEY")</code>, null when unset, so handle it. You met them as the 12-factor way to configure deployments. From the program's side they're one more untyped string arriving from outside. That's the running theme of this stream: <i>args, prompts, env vars: everything from outside is a String until you validate it into something better.</i></p>`,
...
tests:[{d:'Default format seeded before parsing',re:'put\\s*\\(\\s*"format"\\s*,\\s*"text"\\s*\\)'},{d:'--format consumes the NEXT element',re:'("--format"\\s*(->|:)|equals\\s*\\(\\s*"--format"\\s*\\)|"--format"\\s*\\.equals\\s*\\()[^;]{0,80}?\\.put\\s*\\(\\s*"format"\\s*,\\s*\\w+\\s*\\[\\s*(\\+\\+\\s*i|i\\s*\\+\\s*1)\\s*\\]'},{d:'--verbose is a standalone boolean flag',re:'("--verbose"\\s*(->|:)|equals\\s*\\(\\s*"--verbose"\\s*\\)|"--verbose"\\s*\\.equals\\s*\\()[^;]{0,80}?\\.put\\s*\\(\\s*"verbose"\\s*,\\s*"true"'},{d:'Positional argument lands under file',re:'(default\\s*(->|:)|\\belse\\b)[^;]{0,80}?\\.put\\s*\\(\\s*"file"\\s*,'},{d:'Missing file: usage to System.err, not out',re:'System\\.err\\.println\\s*\\(\\s*"usage'},{d:'Failure returns the empty immutable map',re:'return\\s+Map\\.of\\s*\\(\\s*\\)'}],
...
<li><b>Return Optional for expected failure</b>. A user typing junk is ordinary, not exceptional. A helper like <code>Optional&lt;Integer&gt; tryParse(String s)</code> makes the maybe-ness part of the signature. Callers write <code>tryParse(s).orElse(defaultValue)</code> instead of try/catch at every call site.</li>
<li><b>Check ranges and formats after parsing</b>. 9999 parses fine and is still not a valid age. Validation is parse + predicate: <code>age &gt;= 0 &amp;&amp; age &lt;= 130</code>. Use regex for shapes: <code>s.matches("[A-Za-z][A-Za-z0-9_]{2,15}")</code> for a username, anchored by default with matches.</li>
...
<p>That pipeline reads parse → constrain → default. Every raw String funnels through one validating gate that produces either a good value or a clear outcome. That shape is the entire lesson. Programs that sprinkle <code>parseInt</code> everywhere are haunted.</p>`,
...
tests:[{d:'Null handled with a plain check, not a catch',re:'if\\s*\\(\\s*s\\s*==\\s*null\\s*\\)(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?Optional\\.empty'},{d:'Catches ONLY NumberFormatException',re:'catch\\s*\\(\\s*NumberFormatException\\s+\\w+\\s*\\)'},{d:'Broad Exception never caught',re:'catch\\s*\\(\\s*Exception\\b',not:true},{d:'port: parse then filter range then default',re:'port\\s*\\([^)]*\\)\\s*\\{(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?tryParse\\s*\\(\\s*raw\\s*\\)(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?filter\\s*\\(\\s*\\w+\\s*->\\s*\\w+\\s*>=\\s*1\\s*&&\\s*\\w+\\s*<=\\s*65535(?:[^{}]|\\{[^{}]*\\})*?(?:\\{(?:[^{}]|\\{[^{}]*\\})*?)*?orElse\\s*\\(\\s*8080'},{d:'username validated with an anchored matches regex',re:'matches\\s*\\(\\s*"\\[A-Za-z\\]\\[A-Za-z0-9_\\]\\{2,15\\}"'},{d:'Rejection message says what WOULD be right',re:'IllegalArgumentException\\s*\\(\\s*"[^"]*3-16'}],
...
{title:'Parse, do not trust',lang:'js',diff:'medium',
run:{call:'parsePort',cases:[{"name": "a valid port", "args": ["8080"], "expect": 8080}, {"name": "surrounding whitespace is tolerated", "args": ["  443 "], "expect": 443}, {"name": "above the valid range", "args": ["70000"], "expect": null}, {"name": "digits followed by letters is not a number", "args": ["80a"], "expect": null}, {"name": "negative is rejected", "args": ["-1"], "expect": null}, {"name": "empty input is rejected", "args": [""], "expect": null}]},
prompt:`Write <code>function parsePort(raw)</code> returning the port as a <b>number</b> when the input is entirely digits and lies between 1 and 65535, and <code>null</code> otherwise. Trim first. Do not use <code>parseInt</code> alone; it happily reads <code>"80a"</code> as 80, which is exactly the "parse, do not trust" failure this lesson is about.`,
...
tests:[{d:'the whole string is validated',re:'\\^\\\\d\\+\\$|\\^\\[0-9\\]\\+\\$'},{d:'input is trimmed',re:'trim'},{d:'the range is enforced on the value that is returned',re:'return\\s+(?!\\s*!)[^;]{0,80}65535|65535[^;]{0,40}\\)\\s*\\{?\\s*return\\s+null'},{d:'failures return null',re:'return null'}],
...
<p>The details that make it work, each a lesson from earlier in the stream. Read <b>whole lines</b>, then split. Token-mode reads would fight multi-word arguments. <code>split("\\\\s+", 2)</code> keeps everything after the command as ONE argument: "add buy milk" → add, "buy milk". The <code>default</code> branch answers <i>every</i> unrecognized command. Silence is the worst UX. <code>hasNextLine()</code> guards the end of piped input, so the loop also works non-interactively with <code>java App &lt; commands.txt</code>. Your REPL is instantly scriptable and testable. And state (the todos list) lives <i>outside</i> the loop.</p>
<p>A REPL is <b>state + a command dispatch</b>. That's why the same skeleton scales from this toy to database consoles. When commands multiply, the switch graduates to a <code>Map&lt;String, Consumer&lt;String&gt;&gt;</code> of handlers: the strategy pattern from the Design Patterns stream.</p>`,
...
<p>Everything so far assumed the user is clumsy. Security starts when you assume the user is <b>hostile</b>. The promotion from "validates input" to "safe to expose" is learning what hostile input looks like. The dojo has shown you pieces. This lesson assembles the mindset.</p>
<ul>
<li><b>Injection: never concatenate input into another language.</b> A name typed as <code>'; DROP TABLE users; --</code> is data to you and code to your database. The database stream's rule, <code>PreparedStatement</code> with <code>?</code> and never string-built SQL, is one instance of the general law: input goes into <i>parameter slots</i>, never into <i>source text</i>. The same reasoning bans splicing input into shell commands. Use <code>ProcessBuilder</code> with list arguments, never a concatenated command line. It also bans unescaped HTML, the web stream's <b>XSS</b> lesson. XSS is cross-site scripting: an attacker gets their JavaScript to run inside a page you trust, at which point it can read anything that page can, including tokens kept in the browser.</li>
...
<li><b>Allowlist beats blocklist.</b> "Reject <code>&lt;script&gt;</code>" loses to the next encoding trick you didn't think of. "Accept only <code>[A-Za-z0-9_-]{1,64}</code>" can't lose. Validate for what IS allowed. Never enumerate evil.</li>
<li><b>Fail closed, complain vaguely.</b> On invalid input: refuse, log the details server-side, and answer the user generically. Say "invalid filename", not "path /srv/uploads/../etc resolved outside base". That message is a map for the attacker.</li>
</ul>
...
exs:[{title:'Harden the upload gate',
...
{title:'Reject a hostile filename',lang:'js',diff:'hard',
run:{call:'safeFilename',cases:[{"name": "an ordinary filename", "args": ["report-1.txt"], "expect": true}, {"name": "path traversal", "args": ["../etc/passwd"], "expect": false}, {"name": "an absolute path", "args": ["/etc/passwd"], "expect": false}, {"name": "a nested traversal that contains no slash at the start", "args": ["reports/../../secret"], "expect": false}, {"name": "a null byte", "args": ["ok.txt\\u0000.png"], "expect": false}, {"name": "an empty name", "args": [""], "expect": false}]},
prompt:`Write <code>function safeFilename(name)</code> returning <code>true</code> only for names built from letters, digits, dot, underscore and hyphen, with no <code>..</code> sequence anywhere. Everything else (slashes, null bytes, empty strings) is rejected. This is an <b>allowlist</b>: describe what is permitted, never what is forbidden.`,
starter:`function safeFilename(name) {\n  return false;\n}`,
solution:`function safeFilename(name) {\n  return /^[A-Za-z0-9._-]+$/.test(name) && !name.includes("..");\n}`,
tests:[{d:'an anchored allowlist pattern is used',re:'\\^\\[A-Za-z0-9'},{d:'the pattern is anchored at both ends',re:'\\$/'},{d:'traversal sequences are rejected',re:'\\.\\.'},{d:'the allowlist test is what the function returns',re:'(return|=)\\s*(?!\\s*!\\s*\\()[^;]{0,120}\\.test\\s*\\('}],
behavior:`Six cases run. The nested case is the argument for allowlisting: a denylist that checks whether the name STARTS with ".." accepts "reports/../../secret", and every denylist has a case like it that someone finds later. The null-byte case is historical but still worth knowing: in languages with C-backed file APIs, a null byte truncates the string, so "ok.txt\u0000.png" passes an extension check and opens ok.txt. An anchored allowlist excludes all of these at once without anyone having to enumerate them.`,
hints:['Describe what is ALLOWED, anchored at both ends, and reject everything else.','+ rather than * so an empty name fails.','The .. check is belt and braces: the allowlist already excludes slashes, but traversal is worth failing explicitly.']}]}
]});
