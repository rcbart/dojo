...
<p>Three names, three distinct practices. People conflate them:</p>
<ul>
<li><b>Continuous Integration (CI)</b>: every push to the shared branch triggers an automated build and the full test suite. The point is <i>small batches</i>. Integrating twice a day surfaces conflicts and breakage while the change is still small and fresh in your head. No green pipeline → no merge.</li>
<li><b>Continuous Delivery (CD #1)</b>: every commit that passes CI is automatically packaged into a <i>deployable artifact</i> (a versioned jar, a tagged Docker image). Releasing becomes a <b>business decision, not an engineering scramble</b>. A human still presses the button.</li>
<li><b>Continuous Deployment (CD #2)</b>: nobody presses the button. Passing the pipeline <i>is</i> the release. Every green commit rolls to production automatically. This requires serious test coverage, monitoring, and fast rollback.</li>
</ul>
<p>The pipeline is a funnel. Each stage is cheaper to fail in than the next:</p>
...
<p><b>Why it matters</b>: teams that deploy small changes often have <i>fewer</i> incidents, not more. Each deploy carries less risk, and rollback is trivial because the previous artifact is one tag away. The DORA metrics (deploy frequency, lead time, change-failure rate, time-to-restore) all improve together.</p>
<p>Vocabulary for the next lessons. An <b>artifact</b> is the immutable build output. An <b>environment</b> is dev, staging or prod: same artifact, different config. A <b>gate</b> is a manual or automated check between environments. A <b>rollback</b> redeploys the previous artifact. "Fix forward under pressure" is never plan A.</p>`,
...
<h4>Triggers are a security question</h4>
<p>Understand <code>pull_request</code> versus <code>pull_request_target</code> before you copy either.
<code>pull_request</code> runs the workflow <i>from the pull request</i> with no access to your secrets.
<code>pull_request_target</code> runs it <i>from your default branch</i> with secrets available.
That is safe only if it never checks out and executes the contributor's code. Get the pair backwards
on a public repository and anyone who can open a pull request has your secrets.</p>

<h4>Jobs, steps and state</h4>
<p>Steps within a job share a runner and a working directory. <b>Jobs do not.</b> Each job gets a fresh
machine, so anything one job produces and another needs must travel through an artifact or a cache.
Each <code>run:</code> step is its own shell. A <code>cd</code> or an exported variable does not survive
...
download into seconds. Caching compiled classes risks a stale build that passes. Use a matrix only for
versions you support. Every cell is a full run: a three-by-three matrix is nine machines and nine times the minutes.</p>
<p>Pin third-party actions. <code>@v4</code> is a moving tag that can be repointed, so a supply-chain
compromise upstream becomes your CI running someone else's code with your secrets. Pin to a commit SHA.</p>`,
...
hints:['Two triggers = two keys under on:, namely push (with branches: [main] nested) and workflow_dispatch (no value needed, just the key with a colon).','Steps is a YAML list: each - starts a step; a step has either uses: or run: (plus an optional name:).','Context expressions live inside \${{ ... }}; YAML treats the whole thing as a plain string.']}},
...
<li><b>Tag images with the commit SHA</b>: <code>:latest</code> tells you nothing. <code>:\${{ github.sha }}</code> makes every running container traceable to the commit. Ship <code>:latest</code> as an <i>additional</i> tag if you like.</li>
...
<h4>Fast feedback is a design goal</h4>
<p>A pipeline that takes twenty minutes changes behavior. People stop waiting for it, batch their changes,
and merge on hope. Under five minutes it becomes part of the edit-run loop. Cache dependencies, run the fast checks first, and
parallelize the slow ones.</p>
...
<h4>Traceability during an incident</h4>
<p>The question during an incident is "what is running?" <code>:latest</code> cannot answer
it. The digest pinned to a commit can. Stamp the version,
commit and build time into the artifact too. It costs nothing and turns a guess into a lookup.</p>
...
long-lived cloud key: the workload-identity argument from the identity course.</li>
...
being a gate. A red build that gets overridden is the same failure.</li>
</ul>
<p>The first item needs two words unpacked. <b>OIDC</b> is OpenID Connect: a thin layer on top of OAuth that adds the missing piece, a signed statement of who logged in, called an ID token. OAuth answers "what may this app do"; OIDC answers "who is this person". <b>Federation</b> is one side agreeing to trust the other's logins. Here the "person" is the workflow run: GitHub signs a statement saying which repository and branch is running, and the cloud provider trades it for a credential that expires in minutes, so there's no long-lived key to store or steal.</p>`,
...
<li><b>Rolling deploy</b>: replace instances a few at a time behind the load balancer. Zero downtime, but two versions run at once, and your DB migrations must tolerate that.</li>
<li><b>Blue/green</b>: run the full new stack (green) beside the old (blue), flip traffic at the router, keep blue warm for instant rollback.</li>
<li><b>Canary</b>: route 1-5% of real traffic to the new version, watch error rates and latency, then ramp. Production traffic is the one workload you cannot simulate.</li>
...
<p>One rule: <b>the artifact is immutable</b>. If staging tested image <code>:abc123</code>, production runs <code>:abc123</code>, not a rebuild "of the same code". Rebuilds can differ (new base image, new dependency resolution). What you tested is what you ship.</p>

<h4>The database is what makes deployment hard</h4>
<p>Rolling deploys mean two versions of your code run at the same time, against one schema. That dictates the discipline: <b>expand, migrate, contract</b>. Add the new column. Deploy code that writes
both old and new. Backfill. Deploy code that reads the new one. Only then remove the old. Four
deploys, each safe on its own, instead of one that must be perfect.</p>
...
<li><b>Blue-green</b>: full second environment, instant switch, instant rollback. You pay 2× capacity during the cutover and you still have the schema problem.</li>
<li><b>Canary</b>: the only strategy that gives you evidence before full exposure, and the only one that requires
you to decide in advance what "bad" looks like.</li>
<li><b>Feature flags</b>: separate <i>deploying</i> from <i>releasing</i>. The risky change ships dark
...
<p>Rollback counts as a plan once it is a rehearsed action with a number attached. How long does it take? Who can
do it at 3am? Does it survive the migration you just ran? If any answer is unknown, you
have a forward-only deploy and should treat every release accordingly.</p>`,
...
<p>Why teams switch: the git repo becomes the <b>single source of truth</b>. Every change is a reviewable PR. The audit log is git log. Disaster recovery is "point a fresh cluster at the repo". Cluster credentials never sit in CI secrets.</p>
...
<li><b>application-controller</b> compares rendered (desired) state against live cluster state. It reports <b>Sync status</b> (in sync / OutOfSync) and <b>Health</b> (Healthy / Progressing / Degraded), and applies diffs.</li>
...
<p><code>prune</code> and <code>selfHeal</code> are the GitOps contract enforced. Git says it → it exists. Git does not → it goes. Someone hand-edits the cluster → ArgoCD puts it back. "Drift" stops being a slow mystery and becomes a red OutOfSync badge.</p>`,
...
<p>That middle step in Actions is three shell lines. It needs a deploy key or app token for the gitops repo. This one <i>does</i> need a secret, scoped to that repo only:</p>
...
<p>Day-2 ArgoCD you should know: <b>sync waves</b> (<code>argocd.argoproj.io/sync-wave</code> annotations order db-migration Jobs before Deployments), <b>hooks</b> (PreSync migration Jobs), and the <b>CLI</b> for scripting and CI checks. Rollback in GitOps is boring: <code>git revert</code> the bump commit. ArgoCD sees the old tag and converges.</p>

<h4>Why the two halves are separate on purpose</h4>
<p>The pipeline stops at the registry. Nothing in CI holds cluster credentials. A compromised build job cannot deploy, because it has no way to reach production. It can only publish an
artifact and propose a change.</p>
<p>The separation also changes what "deployed" means. Push-based CD reports success when the deploy command
returns. Pull-based CD reports it when the cluster's actual state matches the declared state. That is a
stronger claim, and the reason drift shows up as a status rather than a surprise months later.</p>

<h4>The handover: how the image tag reaches the manifest</h4>
<p>Something has to write the new tag into the manifest repository. The workable options: a CI step that commits the tag to the config repo, or an image updater watching
...
<ul>
<li><b>A rollback is a revert.</b> If going back means re-running a pipeline, you have a slower deploy, not GitOps.</li>
