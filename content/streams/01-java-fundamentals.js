...
tests:[{d:'Declares public class Greeter',re:'public\\s+class\\s+Greeter'},{d:'Has a proper main method',re:'public\\s+static\\s+void\\s+main\\s*\\(\\s*String\\s*\\[\\]\\s*\\w+\\s*\\)'},{d:'Defines static String greet(String ...)',re:'static\\s+String\\s+greet\\s*\\(\\s*String\\s+\\w+\\s*\\)'},{d:'Prints the welcome line',re:'(print(ln|f)?\\s*\\(\\s*"Welcome to Dev Dojo!|=\\s*"Welcome to Dev Dojo!")'}],
behavior:`1. main prints "Welcome to Dev Dojo!" as first line. 2. greet("Ada") returns exactly "Hello, Ada!". 3. main prints the result of greet("Ada"). 4. Code compiles (types, semicolons, braces).`,
...
tests:[{d:'Uses an else-if chain',re:'else\\s+if\\s*\\('},{d:'Checks 90 first (order matters)',re:'if\\s*\\(\\s*score\\s*>=\\s*90'},{d:'No switch used',re:'switch',not:true},{d:'Each threshold returns its own grade',re:'(?=[\\s\\S]*>=\\s*90[^;]{0,40}"A")(?=[\\s\\S]*>=\\s*80[^;]{0,40}"B")(?=[\\s\\S]*>=\\s*70[^;]{0,40}"C")(?=[\\s\\S]*>=\\s*60[^;]{0,40}"D")(?=[\\s\\S]*"F")'}],
...
tests:[{d:'sign is a single ternary return mapping each sign to its word',re:'(?=[\\s\\S]*return\\s+n\\s*[<>=!][^;]*\\?[^;]*:[^;]*;)(?=[\\s\\S]*(<\\s*0\\s*\\)?\\s*\\?\\s*\\(?\\s*"negative"|>\\s*0\\s*\\)?\\s*\\?\\s*\\(?\\s*"positive"|<=\\s*-\\s*1\\s*\\)?\\s*\\?\\s*\\(?\\s*"negative"))'},{d:'Guard throws on bad input',re:'throw\\s+new\\s+IllegalArgumentException'},{d:'Null-checked before use',re:'age\\s*==\\s*null|null\\s*==\\s*age'},{d:'No nested if blocks (flat guards)',re:'if\\s*\\([^)]*\\)\\s*\\{[^}]*if\\s*\\(',not:true}],
...
hints:['Chained ternary: <code>return n < 0 ? "negative" : n == 0 ? "zero" : "positive";</code>. That is acceptable because each branch is trivial.','Guards are one-liners: <code>if (age == null || age < 0) throw new IllegalArgumentException("age must be a non-negative number");</code>','After the guards the happy path is a single ternary or if/return.'],
...
tests:[{d:'switch expression assigned/returned',re:'return\\s+switch\\s*\\(\\s*month\\s*\\)'},{d:'Multi-label arrow case',re:'case\\s+1\\s*,\\s*2\\s*,\\s*3\\s*->'},{d:'Each month group maps to its own quarter',re:'(?=[\\s\\S]*case\\s+1\\s*,\\s*2\\s*,\\s*3\\s*->\\s*(\\{\\s*yield\\s+)?"Q1")(?=[\\s\\S]*case\\s+4\\s*,\\s*5\\s*,\\s*6\\s*->\\s*(\\{\\s*yield\\s+)?"Q2")(?=[\\s\\S]*case\\s+7\\s*,\\s*8\\s*,\\s*9\\s*->\\s*(\\{\\s*yield\\s+)?"Q3")(?=[\\s\\S]*case\\s+10\\s*,\\s*11\\s*,\\s*12\\s*->\\s*(\\{\\s*yield\\s+)?"Q4")'},{d:'default block uses yield',re:'default\\s*->\\s*\\{[\\s\\S]*?yield'},{d:'No break statements',re:'\\bbreak\\s*;',not:true}],
...
hints:['Header: <code>for (int i = 1; i <= n; i++)</code>. Note the inclusive upper bound.','Skip pattern: <code>if (i % 2 != 0) continue;</code> then accumulate below it.','Trace the boundaries: first iteration i=1 (skipped), last i=n.'],
...
tests:[{d:'Uses the String constructor',re:'new\\s+BigDecimal\\s*\\(\\s*\\w+\\s*\\)|new\\s+BigDecimal\\s*\\(\\s*"'},{d:'Never the double constructor (precision trap)',re:'new\\s+BigDecimal\\s*\\(\\s*[0-9]+\\.[0-9]',not:true},{d:'Quantity via BigDecimal.valueOf',re:'BigDecimal\\.valueOf\\s*\\(\\s*quantity\\s*\\)'},{d:'Explicit scale + HALF_EVEN',re:'setScale\\s*\\(\\s*2\\s*,\\s*RoundingMode\\.HALF_EVEN\\s*\\)'},{d:'same() returns compareTo(...) == 0, not equals',re:'return\\s+(?!\\s*!)[^;]{0,60}(compareTo\\s*\\(\\s*\\w+\\s*\\)\\s*==\\s*0|0\\s*==\\s*\\w+\\.compareTo\\s*\\(\\s*\\w+\\s*\\))'}],
...
tests:[{d:'objects live on the heap',re:'(["\']object["\']\\s*(->|:)\\s*(\\{\\s*)?(yield\\s+|return\\s+)?["\']heap["\']|equals\\s*\\(\\s*["\']object["\']\\s*\\)[^"\']{0,40}["\']heap["\']|["\']object["\']\\s*\\.equals\\s*\\([^"\']{0,40}["\']heap["\']|===?\\s*["\']object["\'][^"\']{0,40}["\']heap["\']|["\']object["\']\\s*===?[^"\']{0,40}["\']heap["\']|(Map\\.of|Map\\.ofEntries|entry|\\.put|\\.set)\\s*\\([^;]{0,600}?["\']object["\']\\s*,\\s*["\']heap["\'])'},{d:'local primitives live on the stack',re:'(["\']local\\-primitive["\']\\s*(->|:)\\s*(\\{\\s*)?(yield\\s+|return\\s+)?["\']stack["\']|equals\\s*\\(\\s*["\']local\\-primitive["\']\\s*\\)[^"\']{0,40}["\']stack["\']|["\']local\\-primitive["\']\\s*\\.equals\\s*\\([^"\']{0,40}["\']stack["\']|===?\\s*["\']local\\-primitive["\'][^"\']{0,40}["\']stack["\']|["\']local\\-primitive["\']\\s*===?[^"\']{0,40}["\']stack["\']|(Map\\.of|Map\\.ofEntries|entry|\\.put|\\.set)\\s*\\([^;]{0,600}?["\']local\\-primitive["\']\\s*,\\s*["\']stack["\'])'},{d:'a reference variable lives on the stack',re:'(["\']reference\\-variable["\']\\s*(->|:)\\s*(\\{\\s*)?(yield\\s+|return\\s+)?["\']stack["\']|equals\\s*\\(\\s*["\']reference\\-variable["\']\\s*\\)[^"\']{0,40}["\']stack["\']|["\']reference\\-variable["\']\\s*\\.equals\\s*\\([^"\']{0,40}["\']stack["\']|===?\\s*["\']reference\\-variable["\'][^"\']{0,40}["\']stack["\']|["\']reference\\-variable["\']\\s*===?[^"\']{0,40}["\']stack["\']|(Map\\.of|Map\\.ofEntries|entry|\\.put|\\.set)\\s*\\([^;]{0,600}?["\']reference\\-variable["\']\\s*,\\s*["\']stack["\'])'},{d:'class metadata lives in metaspace',re:'(["\']class\\-metadata["\']\\s*(->|:)\\s*(\\{\\s*)?(yield\\s+|return\\s+)?["\']metaspace["\']|equals\\s*\\(\\s*["\']class\\-metadata["\']\\s*\\)[^"\']{0,40}["\']metaspace["\']|["\']class\\-metadata["\']\\s*\\.equals\\s*\\([^"\']{0,40}["\']metaspace["\']|===?\\s*["\']class\\-metadata["\'][^"\']{0,40}["\']metaspace["\']|["\']class\\-metadata["\']\\s*===?[^"\']{0,40}["\']metaspace["\']|(Map\\.of|Map\\.ofEntries|entry|\\.put|\\.set)\\s*\\([^;]{0,600}?["\']class\\-metadata["\']\\s*,\\s*["\']metaspace["\'])'},{d:'deep recursion throws StackOverflowError',re:'(["\']deep\\-recursion["\']\\s*(->|:)\\s*(\\{\\s*)?(yield\\s+|return\\s+)?["\']StackOverflowError["\']|equals\\s*\\(\\s*["\']deep\\-recursion["\']\\s*\\)[^"\']{0,40}["\']StackOverflowError["\']|["\']deep\\-recursion["\']\\s*\\.equals\\s*\\([^"\']{0,40}["\']StackOverflowError["\']|===?\\s*["\']deep\\-recursion["\'][^"\']{0,40}["\']StackOverflowError["\']|["\']deep\\-recursion["\']\\s*===?[^"\']{0,40}["\']StackOverflowError["\']|(Map\\.of|Map\\.ofEntries|entry|\\.put|\\.set)\\s*\\([^;]{0,600}?["\']deep\\-recursion["\']\\s*,\\s*["\']StackOverflowError["\'])'},{d:'exhausting the heap throws OutOfMemoryError',re:'(["\']too\\-many\\-objects["\']\\s*(->|:)\\s*(\\{\\s*)?(yield\\s+|return\\s+)?["\']OutOfMemoryError["\']|equals\\s*\\(\\s*["\']too\\-many\\-objects["\']\\s*\\)[^"\']{0,40}["\']OutOfMemoryError["\']|["\']too\\-many\\-objects["\']\\s*\\.equals\\s*\\([^"\']{0,40}["\']OutOfMemoryError["\']|===?\\s*["\']too\\-many\\-objects["\'][^"\']{0,40}["\']OutOfMemoryError["\']|["\']too\\-many\\-objects["\']\\s*===?[^"\']{0,40}["\']OutOfMemoryError["\']|(Map\\.of|Map\\.ofEntries|entry|\\.put|\\.set)\\s*\\([^;]{0,600}?["\']too\\-many\\-objects["\']\\s*,\\s*["\']OutOfMemoryError["\'])'},{d:'unknown is the default answer',re:'(default\\s*(->|:)\\s*(\\{\\s*)?(yield\\s+|return\\s+)?["\']unknown["\']|else[^"\']{0,40}["\']unknown["\']|getOrDefault\\s*\\([^;]{0,120}["\']unknown["\']|(\\?\\?|\\|\\|)\\s*["\']unknown["\']|return\\s+["\']unknown["\']\\s*;)'}],
