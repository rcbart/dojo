...
// impure - and each impurity is a real testing cost:
let total = 0;
...
overwrite each other. The discipline that prevents it: <b>treat data as read-only, and make changed
copies</b>.</p>
<div class="codeSample" data-hl>const user = { name: "Ada", visits: 3 };
const bumped = { ...user, visits: user.visits + 1 };   // new object, one change
const tagged = [...tags, "new"];                        // push, without push

// the array methods sort themselves into the same two camps:
MUTATE:   push, pop, splice, sort, reverse
COPY:     map, filter, slice, concat, toSorted, toReversed
...
<div class="codeSample" data-hl>const active  = (users) =&gt; users.filter(u =&gt; u.active);
const names   = (users) =&gt; users.map(u =&gt; u.name);
const sorted  = (list)  =&gt; list.toSorted();

sorted(names(active(users)))     // reads inside-out, runs left to right
...
solution:`function promoteAll(users) {
  const promoted = users.map(user => ({ ...user, role: "admin", logins: user.logins + 1 }));
  return { promoted, original: users };
  // the spread copies every property; the overrides after it win.
