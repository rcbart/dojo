...
behavior:`Seven cases execute and two pin the ordering. Case 3 sends an invalid URL from a rate-limited client and must return 429: you refuse the request before spending anything on parsing it, which is the entire point of a rate limit. And the 404-before-410 order matters because "expired" is only meaningful for a code that exists — reporting 410 for an unknown code tells an attacker which codes are real. The 404-vs-410 distinction is the honest one: 404 means never existed, 410 means existed and is deliberately gone, and a client can stop retrying on the second.`,
